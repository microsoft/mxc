...
The large NanVix MicroVM runtime is not part of the automatically installed
win32-x64 package. Consumers that select `containment: "microvm"` explicitly
install `@microsoft/mxc-sdk-nanvix-win32-x64`; its exact SDK peer dependency
keeps the companion payload compatible with the meta package.
...
- **Package** (`Package.NpmSdk.Job.yml`) stages each artifact into
  ...
  **release-completeness gate** that fails the build unless every
  ...
  packed tarball. The same job also packs explicit companion packages into the
  binary-package artifact.
- **Release** (`1ES.Release.yml`) publishes the **platform and companion binary
  packages first**, then the **meta package last**.
...
- **Register the six package names** under the `@microsoft` npm org with the same
  ...
  which is not shipped, plus `@microsoft/mxc-sdk-nanvix-win32-x64`). The release
  step cannot create org-scoped names that do not yet exist with the right
  permissions. **Recommended preflight:** before the
