...
- Config files can reference schemas via `"$schema"` for editor validation. `scripts/versioning/check-schema-conformance.js` validates the `tests/examples` + `tests/configs` corpus against the dev schema in CI.
