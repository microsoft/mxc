...
- **Wire-field mapping coverage**: every field declared on the wire model must be consumed by the mapping layer (`config_parser.rs` / `models.rs`), so a field cannot be advertised by the generated schema and SDK types while the parser silently drops it. The real guard is destructuring the wire struct without `..` in the mapping (see the "Destructure (no `..`)" comments in `config_parser.rs`), which makes an unmapped field a compile error — prefer that for any mapping you touch. `scripts/versioning/check-wire-mapping-coverage.js` is a CI gate over the rest: it accepts a struct outright when the mapping destructures it exhaustively, and otherwise text-matches each field against code with comments and string literals stripped (a name mentioned only in prose, `..range` syntax, or a `.name(` method call does not count as a use). Its own exclusions are pinned by `check-wire-mapping-coverage.test.js`, since an earlier revision matched raw text and passed on a revision that really did drop two fields. The two documented accept-and-ignore annotations (`$schema`, `_comment`) are allowlisted. Note the codegen gates cannot catch this class at all: they generate *from* the wire model, so an unmapped field regenerates cleanly.
