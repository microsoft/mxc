...
| MicroVM (NanVix) | `wxc-exec.exe` | Windows x64 | `backends/nanvix/runner/src/lib.rs` — feature-gated behind `microvm`; npm consumers explicitly install `@microsoft/mxc-sdk-nanvix-win32-x64`, which carries a compatible executor beside the runtime payload |
...
The SDK ships **no native binaries in the meta `@microsoft/mxc-sdk` package**. Each host's core executor binaries are delivered through one of **five** per-platform packages (`@microsoft/mxc-sdk-<os>-<arch>` for `win32`/`linux` x `x64`/`arm64` plus `darwin-arm64`; Intel macOS is unsupported), listed as exact-pinned `optionalDependencies`, so npm installs only the matching host payload. The large NanVix MicroVM runtime is an explicit companion install, `@microsoft/mxc-sdk-nanvix-win32-x64`, with an exact peer dependency on the SDK version; MicroVM resolution selects the compatible `wxc-exec.exe` co-located in that package. `platform.ts` resolves executors fail-closed: production trusts only `MXC_BIN_DIR` and identity/version-validated packages, while a monorepo checkout additionally prefers staged packages and `src/target` builds. Build scripts stage core binaries under `sdk/node/platform-packages/` and NanVix under `sdk/node/companion-packages/`; `scripts/sync-platform-package-versions.js` keeps package versions, optional-dependency pins, and companion peer pins synchronized. See `.azure-pipelines/README.md` for the binary-packages-first publishing flow and release-completeness gate.
...
All Rust crates use `version.workspace = true` to inherit the version from `src/Cargo.toml` `[workspace.package]`. The npm SDK version in `sdk/node/package.json` and the C# SDK version (`<Version>` in `sdk/dotnet/Microsoft.Mxc.Sdk/Microsoft.Mxc.Sdk.csproj`) must match. Run `node scripts/check-version-sync.js` to validate they are in sync. When bumping the version, update `src/Cargo.toml` (workspace version), `sdk/node/package.json`, and the C# csproj in the same commit, then run `node scripts/sync-platform-package-versions.js` to stamp platform and companion manifests, meta `optionalDependencies`, and companion peer pins to the new version (CI enforces lockstep via `--check`).
