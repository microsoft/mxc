...
- `scripts/ci/prepare-{windows,linux}-host.{ps1,sh}` — per-backend host prep,
  plus an inventory of the workload interpreters. Backend prerequisites are
  installed per job; most workload interpreters come from image provisioning
  scripts outside this repository, while Windows prep installs packaged
  `winapp` and OpenSSL per job.
- `tests/scripts/run_ci_backend_tests.{ps1,sh}` — map a matrix `backend` id to
  the repository's existing backend suite. An unwired id fails loudly rather
...
| MicroVM (NanVix) | `wxc-exec.exe` | Windows | `backends/nanvix/runner/src/lib.rs` — feature-gated behind `microvm`. Exact v0.9 supports fully isolated deny/deny/deny or explicitly unrestricted allow/allow/allow across egress, ingress, and host-loopback. Mixed directional postures and egress rules are rejected: the retained legacy IPv4 filter's implicit exceptions do not implement the directional contract. Runtime proxy is unsupported. |
| Hyperlight | `wxc-exec.exe` | Windows | `backends/hyperlight/common/src/lib.rs` — Hyperlight + Unikraft micro-VM backend |
| IsolationSession | `wxc-exec.exe` | Windows | `backends/isolation_session/common/src/` — feature-gated behind `isolation_session`, experimental, uses the in-proc `Windows.AI.IsolationSession.Preview` `IsoSessionOps` API. Supports one-shot execution via `ScriptRunner` and state-aware provision/start/exec/stop/deprovision via `StatefulSandboxBackend`. Rejects filesystem grants and every supplied `ui` policy because no corresponding restriction primitives exist; omitted UI applies no restriction rather than the schema's default lockdown. One-shot rejects `lifecycle.destroyOnExit=false` and `lifecycle.preservePolicy=true`; state-aware roots reject the whole lifecycle section. Network is inherently unrestricted. Exact v0.9 requires `experimental.isolation_session.acknowledgeUnrestrictedNetwork=true` for one-shot or `experimental.isolation_session.provision.acknowledgeUnrestrictedNetwork=true` for provision. The legacy network marker pair is removed; provision has no `network` section, and an authored empty/restrictive one-shot policy is never erased or overridden. The typed marker remains separate from common policy and source-presence flags. Later phases reject new network policy or repeated acknowledgment and inherit the provisioned posture. State-aware missing acknowledgment and invalid shapes fail as `malformed_request`; representable backend-policy failures retain `policy_validation` for state-aware and the existing one-shot error mapping. Optional `appId` remains provision-only (`PFN:<pfn>` for packaged apps), is preserved in the opaque ID, and is forbidden in the closed one-shot section. Streams stdout/stderr, forwards stdin, and uses ConPTY when wxc-exec stdout is a TTY. Public Rust/C# SDK callers use the state-aware lifecycle, not one-shot run/spawn. See `docs/isolation-session/state-aware-rust.md` and `docs/isolation-session/oneshot.md`. |
| WSLc | `wxc-exec.exe` | Windows | `backends/wslc/common/src/` — feature-gated behind `wslc`, experimental, uses runtime-loaded `wslcsdk.dll` to run Linux containers in a WSL2 VM. Supports one-shot/streaming `WSLContainerRunner` and state-aware `WslcStateAwareRunner`. With no SDK cross-process re-attach, the persistent per-user `wxc-wslc-daemon.exe` owns live handles; phase processes use `daemon_client.rs`, and its apartment-affine worker currently serializes exec. Filesystem mounts are fixed at provision; denied paths nested under a mount are rejected. Exact v0.9 networking accepts isolated deny/deny/deny or explicit unrestricted bridged allow/allow/allow across egress default, ingress default and host-loopback. Mixed postures and per-host rules are refused because no independent firewall primitive exists; omitted directional values default to deny, not inherited allow. Published legacy contracts retain their compatibility behavior. One-shot cooperative proxy uses `runtimeConfig.networkProxy` with bridged networking; state-aware exec supplies that field without restating network posture. Endpoints must be guest-routable; no host-loopback-only restriction is borrowed from ProcessContainer. State-aware provision/start/stop/deprovision reject runtime proxy. Supplied UI is refused by presence because no UI-restriction primitive exists. One-shot rejects preservePolicy=true and destroyOnExit=false; state-aware lifecycle sections are structurally forbidden. Validation occurs before container creation. ID grammar is `wslc:<32-hex>`. Idle teardown remains controlled by `MXC_WSLC_DAEMON_IDLE_TIMEOUT_SECS`/`MXC_WSLC_DAEMON_IDLE_POLL_SECS`. See `docs/wsl/wslc-state-aware.md`. |
...
1. User provides JSON config (file or base64) → exact version/phase probes select a closed `mxc_config_contract` request → `config_deserialize.rs` performs path-aware typed deserialization → adapters and shared `config_parser.rs` normalization produce `ExecutionRequest` (the internal execution model in `models.rs`).
