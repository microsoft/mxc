...
- **Compatibility schema-version source**: `schemas/schema-version.json` remains the source of truth for the legacy compatibility constants (min/maxSupported/state-aware/stable/dev) until the remaining rolling consumers retire. `scripts/versioning/check-schema-versions.js` enforces that the Rust parser, SDK, and schema filenames agree with it; do not hand-edit a schema-version constant without updating the canonical file. See [`docs/versioning.md`](../docs/versioning.md) for the full design.
...
1. Add the field to the applicable closed request root under
   `src/core/mxc_config_contract/src/dev/`. Do not add new fields to the legacy
   rolling `wire::Experimental` wrapper; it remains only as a temporary
   differential/runtime compatibility representation until Phase 11d.
2. Add the exact adapter mapping into the shared runtime representation. An
   internal `ExperimentalConfig` field may remain while execution is gated;
