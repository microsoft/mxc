...
- **Dev schemas**: the in-progress schema lives in [`schemas/dev/`](../schemas/dev) and is generated from the exact Rust development contract — **do not hand-edit it**. Regenerate it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- schema --version 0.10.0-alpha --out schemas/dev/mxc-config.schema.0.10.0-alpha.json`. Published schemas in `schemas/stable/` are immutable and are not regenerated. `scripts/versioning/check-contract-codegen.js` gates the exact development artifacts. See [`docs/schema-codegen.md`](../docs/schema-codegen.md).
- **Generated SDK wire types**: `mxc_schema_support` emits the exact development oracle at `sdk/node/src/generated/v0_10_0_alpha/wire.ts`; regenerate it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- types --version 0.10.0-alpha --out sdk/node/src/generated/v0_10_0_alpha/wire.ts`. Do not hand-edit it. Node conformance tests compare the handwritten SDK types directly with this exact artifact.
- **Exact contract lifecycle registry**: the Rust `mxc_config_contract::registry::CONTRACTS` table is the source of truth for exact version identities, lifecycle state, artifact/source paths, and published schema SHA-256 values. Generate `schemas/contract-registry.generated.json` from it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- registry`; do not hand-edit the generated JSON artifact. `scripts/versioning/check-contract-freeze.js` rejects published-contract removal, identity drift, missing registered surfaces, digest changes, fixture changes, and stale generated registry output.
- **Publication profiles**: the development contract declares a Rust-authored publication profile selecting the stable one-shot root and graduated state-aware provision backends. Selecting a state-aware backend includes the shared start/exec/stop/deprovision roots. Published projections must contain no `experimental` member and must fail rather than omit backend fields whose permanent stable location has not been defined.
- **Registry metadata**: `schemas/contract-registry.generated.json` is the only machine-readable version inventory. Repository validation derives accepted versions and schema paths from it rather than from duplicated min/max metadata.
...
JSON location from the moment they enter the exact development contract. There
is no `experimental` JSON object. Structural availability and execution
authorization are independent: the exact development schema accepts the field,
while `--experimental` or the SDK opt-in controls whether the runtime may use
it. See `docs/authoring-a-new-feature.md` for the full checklist.

1. Add the field to the applicable closed request root under
   `src/core/mxc_config_contract/src/dev/`.
2. Add the exact adapter mapping into the shared runtime representation. An
   internal `DevelopmentConfig` field may remain while execution is gated;
   its name does not define the JSON shape.
3. Guard execution behind `request.experimental_enabled`. Absence of
   authorization must fail explicitly for an experimental backend rather than
   silently changing containment.
4. Keep the field/root out of the Rust publication profile until it graduates.
   Promotion removes the runtime gate and adds the existing permanent shape to
   a later published profile; it does not move the JSON field.
5. Regenerate exact schema/TypeScript artifacts; never modify files in
   `schemas/stable/`.
...
- `mxc_engine` is the **single execution engine** — the one home for "given an `ExecutionRequest`, run it". It owns: run-to-completion backend selection (`run` / `resolve_runner`, covering **all** backends, incl. the Windows ProcessContainer BaseContainer/AppContainer BFS/DACL fallback tiers via `appcontainer_common::dispatcher::dispatch_with_fallback`, and every experimental backend, feature-gated); streaming (`spawn` → `Box<dyn SandboxProcess>`); state-aware lifecycle dispatch (`run_state_aware`, including Windows Sandbox and IsolationSession); host probing (`platform_support` / `PlatformSupport`); and config building (`build_request` / `build_request_with_containment`, `SandboxPolicy` + sections, `available_tools_policy`/`user_profile_policy`/`temporary_files_policy`). Rust policy builders construct the exact contract registered for the declared version before shared semantic validation. It depends on the backend crates (cfg-split: appcontainer/windows_sandbox lifecycle/isolation_session/wslc/nanvix on Windows, bubblewrap/lxc/nanvix on Linux, seatbelt on macOS) so it can't live in `wxc_common`. Both the executor binaries and `mxc-sdk` call into it. `ResolvedRunner` carries the boxed runner plus (Windows only) the optional `DaclManager` guard, so `wxc-exec` can park the guard for its signal handler.
...
The parser deserializes each declared version directly into its exact closed contract. All typed config deserialization goes through `config_deserialize.rs`, which distinguishes syntax errors from typed policy errors and adds the complete JSON path plus source line/column. Adapters map into the neutral internal `wire::MxcConfig` representation for shared semantic normalization; JSON is never deserialized directly into that representation.

State-aware adapters produce a checked `StateAwareInput` containing common fields and a `StateAwareOperation`. Phase is derived from the operation; provision is backend-tagged even with no config, and later operations carry required IDs. `ParsedStateAwareRequest` has private fields and retains no raw JSON/source text. Engine routing and opt-in/build gates precede `state_aware_binding` helpers, which produce `BoundStateAwareRequest<B>` for both lifecycle and streaming dispatch. Config associated types are runtime values, not deserialization targets. Preserve `Option<Config>` and optional-field presence, leave defaults and semantic validation in the backend, and never serialize through JSON to bind.
...
The `SandboxPolicy.version` in the SDK must name an exact registered contract: `0.6.0-alpha`, `0.7.0-alpha`, `0.8.0-alpha`, `0.9.0-alpha`, or `0.10.0-alpha`. The SDK validates this in `sandbox.ts`; an unregistered spelling is rejected even inside the minimum/maximum range. State-aware lifecycle requests require `0.10.0-alpha`. The Rust contract registry owns exact dispatch, and `schemas/contract-registry.generated.json` is its generated machine-readable projection. See `docs/versioning.md` for the full design.
