...
- `mxc` is an **importable library** (the Rust analogue of the SDK's `spawnSandboxFromConfig` with `usePty: false`): `spawn_sandbox_from_config` parses a config, selects the host backend via `mxc::dispatch::select_runner`, runs it **without a pty**, and returns captured stdout/stderr. It also offers a handle-based streaming API — `spawn_sandbox` returns a `wxc_common::sandbox_process::SandboxProcess` (via `mxc::dispatch::spawn_runner`) for persistent bidirectional stdio (`take_stdin`/`take_stdout`/`take_stderr`), `kill()`, and `wait()`. It depends on the backend crates (cfg-split: appcontainer on Windows, bubblewrap on Linux, seatbelt on macOS) — so, like the binaries, it can't live in `wxc_common`. The `wxc`/`lxc`/`mxc_darwin` binaries delegate their shared backend arms to `mxc::dispatch::select_runner`. Output capture is gated by `ExecutionRequest::capture_output` (default `false` = the binaries' streaming behaviour; the library forces it `true`). The streaming `SandboxProcess` / `StreamingRunner` traits live in `wxc_common::sandbox_process` (impls in the backend crates); streaming is implemented for every supported backend — Seatbelt (macOS), Bubblewrap (Linux), and Windows ProcessContainer (AppContainer + BaseContainer).
