...
- **Stable schemas**: released, immutable schemas live in [`schemas/stable/`](../schemas/stable) (one file per released version) — never edit or delete them after release. `scripts/versioning/check-stable-schema-history.js` compares against the explicit PR base and fails on any change to an existing stable file.
- **Dev schema**: the in-progress schema lives in [`schemas/dev/`](../schemas/dev). It is **generated** from the Rust wire model (`src/core/wxc_common/src/wire.rs`) by the `mxc_schema_gen` tool — **do not hand-edit it**. To change the dev schema, edit the wire model and regenerate with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- schemas/dev/mxc-config.schema.<dev>.json`. `scripts/versioning/check-codegen.js` is a CI gate that regenerates the dev schema **and** the SDK wire types and fails if either committed copy drifts. See [`docs/schema-codegen.md`](../docs/schema-codegen.md).
- **Generated SDK wire types**: `sdk/node/src/generated/wire.ts` is **generated** from the same wire model by the `mxc_schema_gen --ts` TypeScript emitter (`wxc_common::ts_emit`, no third-party generator) — **do not hand-edit it**. It is a drift oracle (not public API); the SDK unit test `sdk/node/tests/unit/wire-conformance.test.ts` asserts the hand-written public types in `sdk/node/src/types.ts` conform to it, and the same `scripts/versioning/check-codegen.js` gate fails if the committed file drifts. Regenerate with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- --ts sdk/node/src/generated/wire.ts`.
- **Canonical schema-version source**: `schemas/schema-version.json` — the single source of truth for the schema-version constants (`min`, `maxSupported`, `stateAware`, `stableLatest`, `generatedStableFloor`, and `devSchemaFile`). `scripts/versioning/check-schema-versions.js` enforces that the Rust parser, SDK, ordering constraints, and schema filenames all agree. `scripts/versioning/check-supported-range.js` enforces the single-parser contract: the first generated stable release advances the floor past legacy schemas, and any later breaking release advances `min` to its own major.minor line. See [`docs/versioning.md`](../docs/versioning.md).
- Config files can reference schemas via `"$schema"` for editor validation. `scripts/versioning/check-schema-conformance.js` validates the `tests/examples` + `tests/configs` corpus against the dev schema, and (with `--sdk-emission=required`, run from the SDK Unit Test job after `npm run build`) validates the configs the SDK actually emits for each `(platform, containment)` — mocking `os.platform` to cover all platforms in one run so platform-gated SDK builders can't ship a field the strict parser rejects.
- **Promotion guard**: `schemas/config-stability.json` declares the active experimental sub-keys + tombstones for keys already promoted to stable, plus the `stableContainment` allowlist. `scripts/versioning/check-config-stability.js` compares it with the generated dev schema and the explicit PR-base manifest; it fails CI if a feature leaves `experimental` without becoming a permanent tombstone, gains a stable top-level twin, or is promoted without bumping the schema minor. Register every new experimental key here. See `docs/authoring-a-new-feature.md` "Promoting to Stable".
- **Stable schemas are generated, not hand-authored** (going forward): `scripts/versioning/freeze-stable-schema.js --write <ver>` derives a stable schema from the dev schema at release time (strips the experimental + state-aware surface + non-`stableContainment` values). `--check` proves the dev schema can be frozen; `--check-release <ver>` verifies a newly committed release file exactly matches the generator. Freeze a release separately from advancing the next dev line. Legacy stables (0.4–0.7) stay frozen as historical artifacts.
