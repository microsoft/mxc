...
| `backends/` | Per-OS implementations behind a *backend trait* — two families: **containment backends** (`ScriptRunner`) and **learning-mode capture adapters** (`LearningModeBackend`), the latter grouped under `backends/learning_mode/` | `appcontainer/common`, `windows_sandbox/{daemon,guest,common}`, `isolation_session/{bindings,common}`, `hyperlight/common`, `nanvix/{common,binaries,runner}`, `lxc/common`, `bubblewrap/common`, `wslc/common`, `seatbelt/common`, `learning_mode/{windows,linux,macos}` |
...
- `backends/` hosts **two** kinds of backend, distinguished by the trait they implement — don't conflate them. **Containment backends** implement `ScriptRunner` and provide the sandbox (each ships in a `wxc-exec`/`lxc-exec`/`mxc-exec-mac` binary). **Learning-mode capture adapters** (`backends/learning_mode/{windows,linux,macos}`) implement `LearningModeBackend`; they are *libraries linked into `wxc-exec`* (no binary of their own) that capture access denials behind the cross-platform `learning_mode` dispatcher in `core/`. Both follow the same layering rule: backend crates depend on `core/` foundation crates, never the reverse.
