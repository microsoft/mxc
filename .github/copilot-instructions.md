...
- **Consent is a separate control plane.** Executors expose `--telemetry-consent <status|request|withdraw>`; never route consent maintenance through `--config` / `--config-base64`, add it to `MxcConfig`, or publish a consent JSON schema. `telemetry.enabled` remains only the per-run opt-in.
