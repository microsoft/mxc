...
The SDK ships **no native binaries in the meta `@microsoft/mxc-sdk` package**. Each host's executor binaries are delivered through one of **five** per-platform packages (`@microsoft/mxc-sdk-<os>-<arch>` for `win32`/`linux` × `x64`/`arm64` plus `darwin-arm64`; Intel macOS / `darwin-x64` is **not supported** — `isSupportedPlatformTuple()` rejects it and `getPlatformSupport()` reports it unsupported), listed as exact-pinned `optionalDependencies`; npm's `os`/`cpu` filtering installs only the package matching the consuming host. `platform.ts` resolves the executor **fail-closed** with two modes: in a **production (installed) layout** it trusts ONLY the `MXC_BIN_DIR` override and the identity/version-**validated** installed platform package — it never falls through to `bin/<arch>` or `src/target` paths near `node_modules` (the executor is the sandbox boundary). In a **dev (monorepo) layout** (detected by the sibling Rust workspace) it prefers `MXC_BIN_DIR`, then freshly-built local binaries (`sdk/node/platform-packages/<os>-<arch>`, then `src/target/<triple>/{release,debug}`), then the installed package — so a local Rust build wins. The `build.bat`/`build.sh`/`build-mac.sh` scripts stage binaries into `sdk/node/platform-packages/<os>-<arch>/`, and `scripts/sync-platform-package-versions.js` keeps every platform package and the meta `optionalDependencies` pins locked to the meta version (CI enforces this via `--check` plus a unit-test suite). A non-failing `postinstall` (`sdk/node/scripts/postinstall-check.cjs`) warns when the host's platform package is absent or the host is an unsupported target. See `.azure-pipelines/README.md` for the publishing flow (platform packages first, meta last), the bidirectional release-completeness gate (`scripts/check-release-completeness.js`), and the manual npm-org name registration step. The GitHub Actions workflows (`.github/workflows/Package.NpmSdk.Job.yml`, `SDK.Integration.Test.Job.yml`) mirror the ADO pipeline.
