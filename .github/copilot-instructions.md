...
- **SDK** (`sdk/node/`, `@microsoft/mxc-sdk`) — the public API. Buffered one-shot execution through `spawnSandboxAsync` runs **in process through Koffi and `mxc_ffi`** on Koffi's asynchronous worker pool. The streaming `spawnSandbox` / `spawnSandboxFromConfig` APIs use Koffi and the `mxc_ffi` streaming handle directly, returning `MxcSandboxProcess` with .NET-aligned `standardInput` / `standardOutput` / `standardError`, `waitAsync()`, and no PTY. Both paths adapt the existing `SandboxPolicy` / `ContainerConfig` flow into the private `RequestSpec`; neither launches an executor or exposes the private native ABI through the public SDK. The state-aware lifecycle remains executor-backed at this stage of the migration. Per-(backend, phase) typed state-aware `*Config` interfaces and a branded `SandboxId<C>` live in `sdk/node/src/state-aware-types.ts`. Typed wire-format errors live in `sdk/node/src/errors.ts` (closed `ErrorCode` union plus a single `MxcError` class carrying `code: ErrorCode`, mirroring the Rust `MxcError` shape). Platform detection is in `platform.ts`.
