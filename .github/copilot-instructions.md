...
- **SDK** (`sdk/node/`, `@microsoft/mxc-sdk`) — the public API. The one-shot surface now runs entirely **in-process through `mxc_ffi`**: `spawnSandboxAsync` provides buffered run-to-completion, while the existing `spawnSandbox` and `spawnSandboxFromConfig` entry points provide live pipe-based stdio. They adapt `SandboxPolicy` or `ContainerConfig` into the binding request envelope rather than spawning `wxc-exec.exe`, `lxc-exec`, or `mxc-exec-mac`. `createConfigFromPolicy` and `buildSandboxPayload` remain explicit conversion utilities for authoring / serializing `ContainerConfig`, but the Node SDK no longer exposes one-shot execution of arbitrary executor JSON or PTY / `node-pty` APIs. The state-aware surface (`provisionSandbox` / `startSandbox` / `execInSandboxProcess` / `execInSandboxAsync` / `stopSandbox` / `deprovisionSandbox`, in `sdk/node/src/state-aware.ts`) likewise routes through `mxc_ffi`; per-(backend, phase) typed `*Config` interfaces and a branded `SandboxId<C>` live in `sdk/node/src/state-aware-types.ts`. Typed wire-format errors live in `sdk/node/src/errors.ts` (closed `ErrorCode` union plus a single `MxcError` class carrying `code: ErrorCode`, mirroring the Rust `MxcError` shape). Platform detection and host probing are in `platform.ts`, and telemetry consent also routes through the native binding.

The SDK auto-discovers the `mxc_ffi` shared library by checking `sdk/node/bin/<arch>/` (npm-packaged) and `src/target/<target-triple>/{release,debug}/` (local dev), plus the `MXC_FFI_DIR` override. The `build.bat`/`build.sh`/`build-mac.sh` scripts copy `mxc_ffi` and required backend runtime dependencies into the SDK bin directory; the package/build workflows prune standalone executors and `unix-test-proxy` from the npm payload.
...
| `backends/` | Backend-specific code (one subfolder per containment backend or backend support component) | `process_container/common`, `windows_sandbox/{daemon,guest,common,lifecycle}`, `isolation_session/{bindings,common}`, `learning_mode/windows`, `hyperlight/common`, `nanvix/{common,build_common,binaries,runner}`, `lxc/common`, `bubblewrap/common`, `wslc/common`, `seatbelt/common` |
...
- `learning_mode_windows` (`backends/learning_mode/windows`) is a Windows-only backend support crate for the AppInfo-brokered Learning Mode APIs in `processmodel.dll`. It runtime-resolves the Learning Mode trace exports, owns their typed trace handle, decodes sealed ETL traces through `learning_mode_core`, and process-scopes guarded WPR retention with Windows Trace Relogger using exact job-attested PID/creation/exit `FILETIME` ranges. It depends on `wxc_common` plus `learning_mode_core`; runner integration consumes it from the AppContainer backend layer. The trace contract is `HRESULT Start` + retryable `HRESULT Stop` + infallible `Close`: `Stop` never consumes the trace handle, and every started trace must be closed exactly once (closing without stopping is the early-exit discard path). The AppContainer backend owns the process security-environment exports and combined PSEC + Learning Mode lifecycle in `backends/process_container/common/src/secenv.rs` and `native_capture.rs`. The process security-environment contract is `HRESULT Create` + infallible by-value `Close` and consumes a PSEC FlatBuffer, not the legacy SBOX buffer. Ordinary requests use PSEC 1.0; `processContainer.filesystem.enumeratePaths` and unrestricted host-loopback requests require PSEC 1.1 plus their advertised support capability. Generated PSEC bindings live in `core/generated/process_security_environment_specification`.
