...
- **Wire-field mapping coverage**: every field declared on the wire model must be consumed by the mapping layer (`config_parser.rs` / `models.rs`), so a field cannot be advertised by the generated schema and SDK types while the parser silently drops it. `scripts/versioning/check-wire-mapping-coverage.js` is a CI gate that fails on an unconsumed field; the two documented accept-and-ignore annotations (`$schema`, `_comment`) are allowlisted there. The stronger guard is destructuring the wire struct without `..` in the mapping (see the "Destructure (no `..`)" comments in `config_parser.rs`), which makes an unmapped field a compile error — prefer that for any mapping you touch. Note the codegen gates cannot catch this: they generate *from* the wire model, so an unmapped field regenerates cleanly.
