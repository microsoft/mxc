...
| IsolationSession | `wxc-exec.exe` | Windows | `backends/isolation_session/common/src/` — feature-gated behind `isolation_session`, experimental, uses the in-proc `Windows.AI.IsolationSession.Preview` `IsoSessionOps` API. Supports both one-shot (single-invocation lifecycle, via `ScriptRunner`) and state-aware (multi-invocation provision/start/exec/stop/deprovision, via `StatefulSandboxBackend`) modes. The exact `0.9.0-alpha` roots are the structural boundary: fields excluded from a one-shot or per-phase root fail as `malformed_request` before backend validation. This includes filesystem and `ui` on every state-aware phase, non-exec `process`, policy on phases that do not define it, foreign or mis-slotted experimental blocks, and any one-shot `experimental.isolation_session` payload. Structurally representable requests that violate backend semantics fail as `policy_validation`; on the one-shot executor surface the typed backend variant is carried as `backend_error` with the reason in the message. The backend has no host-folder-sharing primitive and no truthful `ui` posture: the isolation session isolates the *host's* UI from contained code but does not deny UI capabilities inside its own session. An omitted one-shot `ui` applies no restriction, so the schema's default-deny reading does not hold on this backend. One-shot additionally rejects `lifecycle.destroyOnExit=false` and `lifecycle.preservePolicy=true`; state-aware roots exclude the whole `lifecycle` section. The container's network is unrestricted, so provision and one-shot require the canonical unrestricted-network acknowledgment — `network.defaultPolicy=allow` + `network.allowLocalNetwork=true`, no host rules, no proxy, default enforcement. State-aware provision accepts an optional `appId` (a packaged app passes `PFN:<pfn>`), carried verbatim inside the returned `sandboxId`; post-provision roots inherit the provisioned posture and admit only their phase-specific fields. The full honor and error matrix is in `docs/isolation-session/state-aware-rust.md`. Streams stdout/stderr, forwards stdin, and switches to ConPTY mode when wxc-exec's stdout is a TTY for `spawnSandbox` parity. |
