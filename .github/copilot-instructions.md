...
| BaseContainer (OS sandbox API) | `wxc-exec.exe` | Windows | `backends/appcontainer/common/src/base_container_runner.rs` — prefers native enforcement when the host supports the requested policy, then tries compatibility and AppContainer fallbacks without silently dropping policy. Native enforcement supports schema 0.8 directional networking and runtime proxy settings; compatibility paths reject combinations they cannot preserve. `captureDenials` uses native Learning Mode when available and guarded, process-scoped WPR otherwise; explicit `retainEtl` applies only after a terminal wait. |
...
- `learning_mode_windows` (`backends/learning_mode/windows`) is the Windows-only support crate for native Learning Mode and guarded, process-scoped WPR capture. It owns the OS API lifecycles and decodes ETL through `learning_mode_core`; the AppContainer backend consumes it. Generated process-security-environment bindings live in `core/generated/process_security_environment_specification`.
