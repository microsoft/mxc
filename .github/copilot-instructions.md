...
tests, while macOS retains its existing common-crate selection. Azure's Linux
test additions retain its native-x64-only restriction. These unit/contract
gates are distinct from the host-dependent backend lifecycle suites below.
...
|---------|--------|----------|--------|
| AppContainer | `wxc-exec.exe` | Windows | `backends/process_container/common/src/appcontainer_runner.rs` |
| BaseContainer (OS sandbox API) | `wxc-exec.exe` | Windows | `backends/process_container/common/src/base_container_runner.rs` — uses `CreateProcessSecurityEnvironment` with PSEC whenever its runtime probe succeeds and the requested policy is compatible, independent of schema version. PSEC receives schema 0.8 egress filters, runtime proxy, proxy peer identity, and host-loopback configuration. When PSEC is unavailable or policy-incompatible, selection continues to the AppContainer tiers. `captureDenials` uses the complete compatible PSEC + V2 Learning Mode path or pairs an AppContainer fallback tier with guarded WPR using exact handle-attested process scope. Both providers honor explicit `retainEtl` after a terminal wait; abandoned processes discard the trace. |
...
- **SDK** (`sdk/node/`, `@microsoft/mxc-sdk`) — the public API. Buffered one-shot execution through `spawnSandboxAsync` runs **in process through Koffi and `mxc_ffi`** on Koffi's asynchronous worker pool: it adapts the existing `SandboxPolicy` / `ContainerConfig` flow into the private binding request instead of spawning an executor or a dedicated Node `Worker`. The internal native streaming layer (`bindings/streaming.ts` + `MxcSandboxProcess`) wraps `mxc_ffi` handles and is integration-tested, but the public `spawnSandbox` / `spawnSandboxFromConfig` surfaces remain executor-backed until native PTY support can preserve their terminal contract. The state-aware lifecycle also remains executor-backed at this stage of the migration. Per-(backend, phase) typed state-aware `*Config` interfaces and a branded `SandboxId<C>` live in `sdk/node/src/state-aware-types.ts`. Typed wire-format errors live in `sdk/node/src/errors.ts` (closed `ErrorCode` union plus a single `MxcError` class carrying `code: ErrorCode`, mirroring the Rust `MxcError` shape). Platform detection is in `platform.ts`.
...
- Config files can reference schemas via `"$schema"` for editor validation. `scripts/versioning/validate-configs.js` validates each `tests/examples` + `tests/configs` document against the exact registered schema named by its `version`.
...
2. Add the matching adapter from the exact contract into the shared wire representation, add the domain `ExperimentalConfig` field in `models.rs`, then map wire→domain in `config_parser.rs` (use `From` impls beside the domain type for trivial enum/struct conversions)
...
|-----------|---------|----------|
| `core/` | Cross-platform foundation, shared platform support, and per-platform aggregator binaries | `wxc_common/`, `wxc/`, `lxc/`, `mxc_darwin/`, `mxc_engine/`, `mxc-sdk/`, `mxc_pty/`, `mxc_build_common/`, `learning_mode_core/`, `learning_mode_platforms/windows/`, `generated/` |
| `backends/` | Backend-specific code (one subfolder per containment backend or its directly owned support components) | `process_container/common`, `windows_sandbox/{daemon,guest,common,lifecycle}`, `isolation_session/{bindings,common}`, `hyperlight/common`, `nanvix/{common,build_common,binaries,runner}`, `lxc/common`, `bubblewrap/common`, `wslc/common`, `seatbelt/common` |
...
- `learning_mode_windows` (`core/learning_mode_platforms/windows`) is the shared Windows support crate for the AppInfo-brokered Learning Mode APIs in `processmodel.dll`. It runtime-resolves the Learning Mode trace exports, owns their typed trace handle, decodes sealed ETL traces through `learning_mode_core`, and process-scopes guarded WPR retention with Windows Trace Relogger using exact job-attested PID/creation/exit `FILETIME` ranges. It depends on `wxc_common` plus `learning_mode_core`; both the ProcessContainer backend and the host-side `plm` utility consume it. The trace contract is `HRESULT Start` + retryable `HRESULT Stop` + infallible `Close`: `Stop` never consumes the trace handle, and every started trace must be closed exactly once (closing without stopping is the early-exit discard path). The ProcessContainer backend owns the process security-environment exports and combined PSEC + Learning Mode lifecycle in `backends/process_container/common/src/secenv.rs` and `native_capture.rs`. The process security-environment contract is `HRESULT Create` + infallible by-value `Close` and consumes a PSEC FlatBuffer, not the legacy SBOX buffer. Ordinary requests use PSEC 1.0; `processContainer.filesystem.enumeratePaths` and unrestricted host-loopback requests require PSEC 1.1 plus their advertised support capability. Generated PSEC bindings live in `core/generated/process_security_environment_specification`.
...
- `wxc`, `lxc`, and `mxc_darwin` are thin binary crates (`wxc-exec` / `lxc-exec` / `mxc-exec-mac`) that wire up CLI args (`clap`), load/validate config, handle maintenance modes (`--probe`, `--available-backends`, `--delete`, `--setup-*`, `--audit`), and **delegate all backend dispatch to `mxc_engine`**. They contain no `match request.containment` of their own. `wxc-exec` additionally owns the Windows Ctrl-C / DACL-cleanup / telemetry orchestration around the engine call. Its `--audit` compatibility workflow synthesizes allow-mode `captureDenials` with ETL retention, then generates policy-authoring artifacts from the actionable denials document returned by the selected engine backend. `--probe` (Windows only) emits the AppContainer diagnostics object, including per-request `FilesystemDeniedPaths` / `FilesystemEnumeratePaths` / `IngressHostLoopbackAllow` capability details; `--available-backends` (Linux only, `lxc-exec`) emits the `mxc_engine::available_backends()` array and is read-only with respect to host state. The two are deliberately distinct flags because their payload shapes differ — see `docs/backend-support-probe-api-plan.md` §6.
