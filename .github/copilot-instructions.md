...
| IsolationSession | `wxc-exec.exe` | Windows | `backends/isolation_session/common/src/` — feature-gated behind `isolation_session`, experimental, uses the in-proc `Windows.AI.IsolationSession.Preview` `IsoSessionOps` API. Supports one-shot execution via `ScriptRunner` and state-aware provision/start/exec/stop/deprovision via `StatefulSandboxBackend`. Rejects filesystem grants because it has no host-folder-sharing primitive, and every supplied `ui` policy because it cannot restrict the contained session's UI capabilities. UI presence is tracked through `ContainerPolicy::ui_specified`; omitted UI applies no restriction rather than the schema's default lockdown. One-shot also rejects `lifecycle.destroyOnExit=false` and `lifecycle.preservePolicy=true`; state-aware roots reject the whole lifecycle section. Network is inherently unrestricted and cannot be filtered or denied. Exact v0.9 accepts `experimental.isolation_session.acknowledgeUnrestrictedNetwork=true` for one-shot or `experimental.isolation_session.provision.acknowledgeUnrestrictedNetwork=true` for provision, with no authored network section. During 10a the canonical legacy acknowledgment (`network.defaultPolicy=allow` plus `network.allowLocalNetwork=true`, no host rules/proxy/non-default enforcement) remains valid alone or consistently with the new field. Supplying neither acknowledgment is rejected; an explicit empty or restrictive network policy is never erased or overridden. The true-only marker is carried separately from common policy, whose implicit defaults and source-presence flags are preserved. Later phases reject new network policy or repeated acknowledgment and inherit the provisioned posture. State-aware missing-both and invalid field shapes fail structurally as `malformed_request`; representable backend-policy failures retain `policy_validation` for state-aware and the existing one-shot error mapping. Optional `appId` remains provision-only (`PFN:<pfn>` for packaged apps), is preserved in the opaque sandbox ID, and is not accepted by the closed one-shot acknowledgment section. Streams stdout/stderr, forwards stdin, and uses ConPTY when wxc-exec stdout is a TTY. Public Rust SDK callers may also drive one-shot `run`/`spawn_sandbox` on this backend; the C# SDK uses the state-aware lifecycle. See `docs/isolation-session/state-aware-rust.md` and `docs/isolation-session/oneshot.md`. |
