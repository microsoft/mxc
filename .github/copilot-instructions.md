...
- **Promotion guard**: `schemas/config-stability.json` declares the active experimental sub-keys + tombstones for keys already promoted to stable, plus the `stableContainment` allowlist. `scripts/versioning/check-config-stability.js` diffs it against the generated dev schema and fails CI if a feature leaves `experimental` without becoming a tombstone, gains a stable top-level twin, or is promoted without bumping the schema minor. Register every new experimental key here. See `docs/authoring-a-new-feature.md` "Promoting to Stable".
- **Stable schemas are generated, not hand-authored** (going forward): `scripts/versioning/freeze-stable-schema.js --write <ver>` derives a stable schema from the dev schema at release time (strips the experimental + state-aware surface + non-`stableContainment` values). `--check` runs in CI to prove the dev schema can be frozen cleanly. Legacy stables (0.4–0.7) stay frozen as historical artifacts.
