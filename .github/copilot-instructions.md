...
- **Promotion guard**: `schemas/config-stability.json` declares the active experimental sub-keys + tombstones for keys already promoted to stable. `scripts/versioning/check-config-stability.js` diffs it against the generated dev schema and fails CI if a feature leaves `experimental` without becoming a tombstone, gains a stable top-level twin, or is promoted without bumping the schema minor. Register every new experimental key here. See `docs/authoring-a-new-feature.md` "Promoting to Stable".
