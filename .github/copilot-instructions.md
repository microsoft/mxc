...
The `SandboxPolicy.version` in the SDK must match a JSON schema version in the supported range (`0.6.0-alpha` minimum, `0.9.0-alpha` maximum). The SDK validates this in `sandbox.ts` — if the policy version is older than `MIN_VERSION` or newer than `SUPPORTED_VERSION` it throws. State-aware lifecycle requests default per backend: `0.6.0-alpha` for `isolation_session` and `windows_sandbox`, `0.8.0-alpha` for `lxc` and `wslc`, whose network surfaces need directional enforcement (`DEFAULT_STATE_AWARE_VERSION` in `sdk/node/src/state-aware-helper.ts`). These bounds are mirrored from the canonical `schemas/schema-version.json` and enforced by `scripts/versioning/check-schema-versions.js`. See `docs/versioning.md` for the full design.
