...
- **SDK** (`sdk/node/`, `@microsoft/mxc-sdk`) — the public API. Buffered one-shot execution through `spawnSandboxAsync` runs **in process through Koffi and `mxc_ffi`** on Koffi's asynchronous worker pool. Streaming `spawnSandbox` / `spawnSandboxFromConfig` preserve the original default PTY contract by launching a packaged Node worker under `node-pty`; the worker calls the attached `mxc_ffi` run entry point, and the backend inherits its terminal without launching an MXC executor binary. Passing `usePty: false` uses the direct `mxc_ffi` streaming handle and returns `MxcSandboxProcess` with .NET-aligned `standardInput` / `standardOutput` / `standardError`, `waitAsync()`, plus `stdin` / `stdout` / `stderr` aliases and ChildProcess-style `exit` / `close` events. Both paths adapt the existing `SandboxPolicy` / `ContainerConfig` flow into the private `RequestSpec`; neither exposes the private native ABI through the public SDK. The state-aware lifecycle remains executor-backed at this stage of the migration. Per-(backend, phase) typed state-aware `*Config` interfaces and a branded `SandboxId<C>` live in `sdk/node/src/state-aware-types.ts`. Typed wire-format errors live in `sdk/node/src/errors.ts` (closed `ErrorCode` union plus a single `MxcError` class carrying `code: ErrorCode`, mirroring the Rust `MxcError` shape). Platform detection is in `platform.ts`.
