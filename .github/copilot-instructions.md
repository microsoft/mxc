...
- **Stable schemas**: released, immutable schemas live in [`schemas/stable/`](../schemas/stable) (one file per released version) — never edit them after release. `scripts/versioning/check-contract-codegen.js` compares every pre-existing stable schema with the merge base and validates current published registry identities.
- **Dev schemas**: the in-progress schemas live in [`schemas/dev/`](../schemas/dev) and are generated by `mxc_schema_gen` — **do not hand-edit them**. The rolling `0.10.0-dev` artifact comes from `src/core/wxc_common/src/wire.rs` and remains a differential migration oracle; regenerate it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- schema --legacy-wire --out schemas/dev/mxc-config.schema.0.10.0-dev.json`. The authoritative exact `0.10.0-alpha` artifact comes from the closed contract in `src/core/mxc_config_contract/src/dev/`; regenerate it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- schema --version 0.10.0-alpha --out schemas/dev/mxc-config.schema.0.10.0-alpha.json`. Published schemas in `schemas/stable/` are immutable and are not regenerated. The rolling and exact development codegen gates fail when either committed artifact drifts. See [`docs/schema-codegen.md`](../docs/schema-codegen.md).
- **Generated SDK wire types**: `mxc_schema_support` provides the shared Rust TypeScript emitter. The rolling drift oracle is `sdk/node/src/generated/wire.ts`; regenerate it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- types --legacy-wire --out sdk/node/src/generated/wire.ts`. The authoritative exact development oracle is `sdk/node/src/generated/v0_10_0_alpha/wire.ts`; regenerate it with `cargo run --manifest-path src/Cargo.toml -p mxc_schema_gen -- types --version 0.10.0-alpha --out sdk/node/src/generated/v0_10_0_alpha/wire.ts`. Published `sdk/node/src/generated/v0_9_0_alpha/wire.ts` is a versioned raw-config oracle, not a frozen runtime snapshot. `scripts/versioning/check-sdk-types-codegen.js` gates the rolling oracle, `scripts/versioning/check-contract-codegen.js` gates exact development artifacts and stable-schema history, and SDK conformance tests guard both surfaces.
...
- `docs/isolation-session/oneshot.md` — IsolationSession backend, one-shot bringup (isolated user account per execution via the OS-side service)
...
New features use their intended permanent JSON locations in the exact development
contract. JSON location, publication eligibility, and runtime authorization are
separate: an ungraduated feature remains active only when `--experimental` is
passed. See `docs/authoring-a-new-feature.md` for the full checklist. The pattern:

1. While the rolling SDK/codegen oracle and exact development contract coexist, add the field to both `src/core/wxc_common/src/wire.rs` and the matching closed request type under `src/core/mxc_config_contract/src/dev/`. Regenerate the rolling and exact schemas and TypeScript oracles with the `mxc_schema_gen schema|types --legacy-wire|--version ... --out ...` commands above — never hand-edit the generated artifacts.
...
- `mxc-sdk` is the **public Rust SDK** — a thin facade over `mxc_engine`.
  ...
  WSLC SDK exposes neither), and IsolationSession (Windows — needs the crate's `isolation_session`
  feature but no runtime experimental opt-in; `id() == 0`, and `kill()` stops
  the whole session).
...
The `SandboxPolicy.version` in the SDK must name an exact registered contract: `0.6.0-alpha`, `0.7.0-alpha`, `0.8.0-alpha`, `0.9.0-alpha`, or `0.10.0-alpha`. The SDK validates this in `sandbox.ts`; an unregistered spelling is rejected even inside the minimum/maximum range. IsolationSession state-aware requests use `0.9.0-alpha`; Windows Sandbox and WSLC use `0.10.0-alpha`. The Rust contract registry owns exact dispatch; `schemas/schema-version.json` and `scripts/versioning/check-schema-versions.js` still synchronize compatibility constants, not runtime range acceptance. Keep the one-shot public `version` property a string with runtime exact validation; generated public registry metadata is separate work. See `docs/versioning.md` for the full design.
