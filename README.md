...
cli/            CLI tool (compiled TypeScript)
...
| `0.5.0-alpha` (strict) | Stable; rejects deprecated v0.4 aliases (CI/local validation mode) | [`schemas/stable/mxc-config.schema.0.5.0-alpha-strict.json`](schemas/stable/mxc-config.schema.0.5.0-alpha-strict.json) |
...
For troubleshooting AppContainer isolation, use Event Tracing for Windows (ETW) to capture access check events. We recommend [Windows Performance Analyzer](https://apps.microsoft.com/detail/9n0w1b2bxgnz) and XPerf.

```powershell
# Start tracing (Administrator)
xperf -on PROC_THREAD+LOADER
xperf -start user -on a68ca8b7-004f-d7b6-a698-07e2de0f1f5d:::'stack'

# Run your sandbox...

# Stop and merge traces
xperf -stop user
xperf -stop
