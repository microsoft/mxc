...
for %%T in (x86_64-pc-windows-msvc aarch64-pc-windows-msvc) do (
    ...
    if exist "!BIN_DIR!\wxc-exec.exe" (
        ...
        if exist "!BIN_DIR!\plm.exe" (
            copy /Y "!BIN_DIR!\plm.exe" "!DEST!\" >nul || goto :error
            echo   Copied win32-!SDK_ARCH!\plm.exe
        )
        if "%WITH_NANVIX%"=="1" if "%%T"=="x86_64-pc-windows-msvc" (
            set "NANVIX_DEST=sdk\node\companion-packages\nanvix-win32-x64"
            if not exist "!NANVIX_DEST!" mkdir "!NANVIX_DEST!"
            for %%F in ("!NANVIX_DEST!\*") do (
                if /I not "%%~nxF"=="package.json" if /I not "%%~nxF"=="README.md" del /Q "%%F" >nul 2>&1
            )
            if exist "!NANVIX_DEST!\bin" rmdir /S /Q "!NANVIX_DEST!\bin"
            if exist "!NANVIX_DEST!\snapshots" rmdir /S /Q "!NANVIX_DEST!\snapshots"
            copy /Y "!BIN_DIR!\wxc-exec.exe" "!NANVIX_DEST!\" >nul || goto :error
            echo   Copied nanvix-win32-x64\wxc-exec.exe
            for %%B in (nanvixd.exe nanvix_rootfs.img python3.initrd) do (
                copy /Y "!BIN_DIR!\%%B" "!NANVIX_DEST!\" >nul || goto :error
                echo   Copied nanvix-win32-x64\%%B
            )
            if not exist "!NANVIX_DEST!\bin" mkdir "!NANVIX_DEST!\bin"
            copy /Y "!BIN_DIR!\bin\kernel.elf" "!NANVIX_DEST!\bin\" >nul || goto :error
            echo   Copied nanvix-win32-x64\bin\kernel.elf
            if not exist "!NANVIX_DEST!\snapshots" mkdir "!NANVIX_DEST!\snapshots"
            for %%S in (kernel.vmem kernel.whp.cbor) do (
                copy /Y "!BIN_DIR!\snapshots\%%S" "!NANVIX_DEST!\snapshots\" >nul || goto :error
                echo   Copied nanvix-win32-x64\snapshots\%%S
            )
        )
        if "%WITH_WSLC%"=="1" (
            copy /Y "!BIN_DIR!\wxc-wslc-daemon.exe" "!DEST!\" >nul || goto :error
            echo   Copied win32-!SDK_ARCH!\wxc-wslc-daemon.exe
