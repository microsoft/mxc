...
filesystem, network, and UI-restriction policy surfaces across the Nickel,
Germanium, and Germanium+ generations.

For the enforcement mechanisms themselves see the
[UI policy schema](./UIPolicy_Schema.md), the
[sandbox policy spec](../sandbox-policy/v1/policy.md), and the
[Playground limitations](../playground-limitations.md) (which documents the
same behaviour from the Playground's point of view).

## OS-version → codename mapping

| Short | Codename | Windows release | Build(s) |
|-------|----------|-----------------|----------|
| **Ni** | Nickel | 22H2 / 23H2 | 22621 / 22631 |
| **Ge** | Germanium | 24H2 | 26100 |
| **Ge+** | Germanium+ | 25H2+ | 26200 / 26600+ |

> **Product floor:** the [README](../../README.md#platforms) and
> [SDK README](../../sdk/README.md) state that `processcontainer`'s **minimum
> supported build is 26100 (Ge / 24H2)**. The Rust code nonetheless build-gates
> individual capabilities down through Ni (22621) and even Windows Server 2022
> (20348). The **Ni** column below therefore describes *what the code can
> enforce if run there* — it is below the officially supported floor and is not
...
- **T1 (BaseContainer)** requires `processmodel.dll` to export
  ...
  is a Ge+ capability. Usability is resolved up front by
  `BaseContainerRunner::is_base_container_usable()` so tier selection never picks
  a T1 that cannot launch.
- **T2 (BFS)** is compiled out by default. `bfscfg.exe` ships only on
  Germanium+ (24H2/25H2), but the `tier2_bfs` Cargo feature is **off** in all
  shipping builds because invoking `bfscfg.exe` can deadlock the host on 25H2.
  Treat T2 as unavailable.
...
Notes:
- On Ge+, T1 can grant `readwrite`/`readonly` paths natively via the FlatBuffer
...
- On Ni/Ge (and on Ge+ hosts where T1 is unavailable), all filesystem policy —
  grants **and** denies — is enforced by T3 host-path DACLs.
...
| Proxy via AppContainer shim (`winhttp-proxy-shim.exe`, needs admin) | ✅ | ✅ | ✅ |
...
- OS-configured WinHTTP proxy (passed in the FlatBuffer spec to
  `CreateProcessInSandbox`) is a T1-only path and therefore Ge+ only. See the
  [Playground limitations](../playground-limitations.md#network-limitations)
  for the DNS/proxy caveats that apply to both AppContainer and BaseContainer.
...
The single UI differentiator for Ni is **`injection`**
(`JOB_OBJECT_UILIMIT_INJECTION`), which the kernel accepts only on build 26100
and later (`MIN_BUILD_FOR_INJECTION_LIMIT`). `ime`
(`JOB_OBJECT_UILIMIT_IME`) requires build 22621 (`MIN_BUILD_FOR_IME_LIMIT`), so
it is available across all of Ni but is dropped on Windows Server 2022 (20348).
