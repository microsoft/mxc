...
### 4. New Crate: `mxc-alpha-bwrap-common`
...
```toml
[package]
name = "mxc-alpha-bwrap-common"
version = "0.1.0"
edition = "2021"

[dependencies]
mxc-alpha-wxc-common = { workspace = true }
mxc-alpha-lxc-common = { workspace = true }
nix = { workspace = true }
...
Add `mxc-alpha-bwrap-common` to workspace `members` in `src/Cargo.toml`.
...
Add `mxc-alpha-bwrap-common` dependency to `lxc/Cargo.toml`.
...
allow/block filtering, the runner reuses `NetworkIptablesManager` from `mxc-alpha-lxc-common` — the
...
`mxc-alpha-lxc-common` (add `mxc-alpha-lxc-common` as a dependency of `mxc-alpha-bwrap-common`). The lifecycle mirrors
...
| Network policy | Low | Reuse `NetworkIptablesManager` from `mxc-alpha-lxc-common`; `--unshare-net` for full block |
...
### Rust (modify)
- `src/Cargo.toml` — add `mxc-alpha-bwrap-common` to workspace members + dependencies
- `src/core/lxc/Cargo.toml` — add `mxc-alpha-bwrap-common` dependency
