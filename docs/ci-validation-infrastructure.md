...
| `windows-canary` | windows | *(dormant)* | *(dormant)* | process, wslc, windows-sandbox, microvm, hyperlight |
...
A backend id is passed straight through: the matrix job hands it to the
host-prep script and then to the dispatcher, which has one `switch`/`case` per
id. The `process` case requires `wxc-exec --probe` to report an enabled native
PSEC contract, then runs a filesystem smoke test against the downloaded
artifact.
...
Resolved `nightly` today = **18 jobs**: 8 Windows (prerelease/25H2/24H2 × process + wslc;
prerelease × isolation-session; 23H2 × wslc),
...
|---------|--------|-------|
| ProcessContainer | ✅ Good | Prerelease Windows with an enabled native PSEC contract. |
| Bubblewrap | ✅ Good | |
