...
The initial `mxc_ffi` wrapper will place each sandbox and stream behind a Rust `Mutex` and use `Mutex::try_lock`.
UniFFI, Node, .NET, and `mxc-sdk` do not provide this behavior. A conflicting call will therefore return a typed busy
error instead of blocking the foreign runtime thread.
