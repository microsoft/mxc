...
|---|---|---|
| Existing operation behavior | Rust implementation and tests | Nothing |
| New callable operation | Rust behavior, one UniFFI export, and value conversion | TypeScript/C# APIs and interop |
| New result or error field | Rust UniFFI record plus its Rust conversion | TypeScript and C# fields and converters |
...
`scripts\generate-uniffi-bindings.ps1` is MXC's complete generation command; contributors do not invoke the individual
generators. Generator and runtime versions are pinned and upgraded together. An upgrade is accepted only when the
generated diff is reviewed and the same ownership, async, stream, error, and process-lifecycle tests pass.
