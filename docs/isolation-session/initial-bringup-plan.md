...
access to the private WinMD. The `windows` crate, `windows-bindgen`, and
the generated `bindings.rs` must all stay on the same release line — a
mismatch trips the build-time check below.
