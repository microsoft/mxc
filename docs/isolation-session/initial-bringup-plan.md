...
| `src/isolation_session_bindings/src/bindings.rs` | Generated by `windows-bindgen` (committed) |
| `src/wxc_common/src/isolation_session_runner.rs` | `IsolationSessionManager` + `IsolationSessionRunner` |
| `tools/generate-isolation-session-bindings/Cargo.toml` | Standalone generator crate (own workspace) |
| `tools/generate-isolation-session-bindings/src/main.rs` | Takes a WinMD path, runs `windows-bindgen` to produce `bindings.rs` |
...
| `src/wxc_common/Cargo.toml` | Add optional dependency on `isolation_session_bindings` |
| `src/wxc_common/src/lib.rs` | Add `pub mod isolation_session_runner` (cfg-gated) |
| `src/wxc_common/src/models.rs` | Add `IsolationSession` to `ContainmentBackend`; add `IsolationSessionConfig` |
| `src/wxc_common/src/config_parser.rs` | Parse `"isolation_session"` containment and the `experimental.isolation_session` section |
| `src/wxc/Cargo.toml` | Add `isolation_session` Cargo feature |
| `src/wxc/src/main.rs` | Dispatch `IsolationSession` behind `--experimental`; call `CoInitializeEx(COINIT_MULTITHREADED)` at top of `main` (required for any WinRT activation, benign for other backends) |
...
`src/isolation_session_bindings/src/bindings.rs` is a checked-in artifact.
...
moves), regenerate via:

```
cargo run --manifest-path tools/generate-isolation-session-bindings/Cargo.toml -- <winmd-path>
```

The generator reads the WinMD, filters to the
`Windows.AI.IsolationEnvironment.Session` namespace plus referenced shared
types, and rewrites `bindings.rs`. The provenance TOML is updated by the
developer manually after regeneration (OS branch, commit SHA, WinMD SHA256,
generator version, generated date) — this is an explicit step to prevent a
stale or ambiguous provenance from sneaking into a commit.

**`build.rs` version check.** `isolation_session_bindings/build.rs` reads
the expected `windows` crate version from `GENERATION_INFO.toml` and
compares against the actual workspace `Cargo.lock`. A mismatch panics the
build with an actionable error message ("regenerate the bindings with the
matching `windows` crate version").
