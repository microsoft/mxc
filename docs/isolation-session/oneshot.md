...
The runner calls into the IsolationSession WinRT APIs exposed by the OS-side
Isolation Session service (running as SYSTEM via `svchost.exe`). Provisioning
and lifecycle management use the stable `Windows.AI.IsolationSession.Preview`
surface. Process launch uses `Windows.AI.IsolationSession` so MXC can populate
the IsoTask-aware `ParentTaskId` and `TaskDisplayName` process options. The APIs
are gated on an internal Windows feature flag.

Activation goes through the WinRT activation factories for both
`IsoSessionOps` runtime classes.
...
`src/backends/isolation_session/bindings/src/bindings.rs` and
`official_bindings.rs` are checked-in artifacts. The WinMD files themselves
are **not** committed (binary, frequently updated). All provenance lives in
...
`windows` crate. The generated bindings cover both `Windows.AI.IsolationSession.Preview` and
`Windows.AI.IsolationSession`; the projection-to-file mapping is recorded in
`GENERATION_INFO.toml`. The build-time check below catches the most common slip
— bumping the workspace `windows` crate without regenerating — by comparing
the workspace version against the recorded `target_windows_crate`.
...
- `process.timeout` (forwarded to the OS-side per-process timeout
  enforcement).
- `process.parentTaskId` and `process.taskDisplayName` (forwarded to the
  broker-created IsoTask process node).
