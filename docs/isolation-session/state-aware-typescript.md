...
| `acknowledgeUnrestrictedNetwork` | `true` | — (**required**) | Explicit acknowledgment that IsolationSession networking cannot be filtered or denied. The SDK nests it under `experimental.isolation_session.provision`. The old network marker pair is not accepted by v0.9, and neither a network section nor acknowledgment redeclaration is accepted on later phases. |
