...
| # | Item | Status | Description | Effort |
|---|---|---|---|---|
| 6 | **(D3) Delegation check** | ✅ Addressed | Policy grants are bounded by the invoking user's access: shared `check_delegation()` in `wxc_common` (`filesystem_access.rs`) verifies the user can read/write each **checkable** listed path before accepting the config, wired into all three runners. Done in [PR #598](https://github.com/microsoft/mxc/pull/598). **Caveat:** paths whose access can't be determined — notably genuinely non-existent paths — are **skipped, not rejected** (`filesystem_access.rs:182-194`); those receive only the parser's advisory existence warning, so a missing entry is not access-checked. | M |
...
| # | Item | Status | Description | Effort |
|---|---|---|---|---|
| 19 | **(N5) Proxy — env-var injection** | ✅ Addressed | `build_process_env()` (`policy_mapping.rs`) injects `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY` (+ lowercase) pointing at the configured proxy; wired into `WslcSetProcessSettingsEnvVariables` (`wsl_container_runner.rs`). Mirrors the Bubblewrap/Seatbelt cooperative model. | — |
...
| 25 | **(N5) Proxy — env-var hygiene** | 🟢 Mostly addressed | `build_process_env()` strips every caller-supplied `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY`/`NO_PROXY` (+ lowercase) from `request.env` before injecting, so a config cannot supply conflicting entries or re-enable a `NO_PROXY` bypass. **Deviation:** `FTP_PROXY`/`ftp_proxy` are *not* stripped — the key list deliberately mirrors Bubblewrap's and Seatbelt's `PROXY_ENV_KEYS` so all three cooperative-proxy backends stay identical; adding `FTP_PROXY` should be one change across all three. | S |
...
>
> **✅ #19/#25 implemented.** `config_parser.rs` now accepts `network.proxy` for `containment: "wslc"`, and `policy_mapping::build_process_env()` strips every caller-supplied proxy variable (`HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, `NO_PROXY` + lowercase — but *not* `FTP_PROXY`, see #25) before injecting the configured URL into `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY` (+ lowercase) via the existing `WslcSetProcessSettingsEnvVariables` path. `NO_PROXY` is deliberately never re-injected, matching Bubblewrap/Seatbelt.
>
...
>
> **⚠️ WSLC-specific wrinkle — NAT reachability.** Unlike Bubblewrap (which shares the host's network namespace, so the container's `127.0.0.1` *is* the host's), the WSLC container runs in the WSL2 VM — a separate kernel with its **own** loopback, behind a NAT. `127.0.0.1` is always machine-local and never routed, so `HTTP_PROXY=127.0.0.1:8080` points at the *container's own* empty loopback, not the host where the proxy listens. **Handled by rejection, not rewriting:** `config_parser.rs` rejects `network.proxy.localhost`, any `url` whose host is local-only (the whole `127.0.0.0/8` block, `::1`, IPv4-mapped `::ffff:127.0.0.1`, the unspecified `0.0.0.0`/`::`, and `localhost` — parsed via `IpAddr::is_loopback()`/`is_unspecified()` rather than matched against literal spellings), and `builtinTestServer` for `wslc`, so a config that cannot work fails fast instead of silently injecting an unreachable address. Rewriting loopback to the VM-visible gateway IP was rejected as a fix because it would assume the consumer's proxy is bound to `0.0.0.0` rather than `127.0.0.1` — a silent, unverifiable assumption. The consumer must supply a `url` that is routable from the VM and bind the proxy on a VM-reachable interface. Also rejected: `defaultPolicy: "block"` (maps to `NetworkingMode::None` — no interface at all, so nothing could reach the proxy) and `allowedHosts`/`blockedHosts` alongside `proxy` (MXC does not forward host lists to an external proxy, and WSLC cannot enforce them itself).
>
> **Net:** #19 + #25 shipped a *cooperative-only* proxy that a rogue app bypasses — the runner logs a warning saying so. The GA-meaningful guarantee (unbypassable model 2) still needs #20 (SDK-blocked).
