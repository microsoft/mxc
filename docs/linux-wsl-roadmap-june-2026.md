...
>
> **⚠️ WSLC-specific wrinkle — NAT reachability.** Unlike Bubblewrap (which shares the host's network namespace, so the container's `127.0.0.1` *is* the host's), the WSLC container runs in the WSL2 VM — a separate kernel with its **own** loopback, behind a NAT. `127.0.0.1` is always machine-local and never routed, so `HTTP_PROXY=127.0.0.1:8080` points at the *container's own* empty loopback, not the host where the proxy listens. **Handled by rejection, not rewriting:** `config_parser.rs` rejects `network.proxy.localhost`, any loopback `url` (`127.0.0.1`/`::1`/`[::1]`/`localhost`), and `builtinTestServer` for `wslc`, so a config that cannot work fails fast instead of silently injecting an unreachable address. Rewriting loopback to the VM-visible gateway IP was rejected as a fix because it would assume the consumer's proxy is bound to `0.0.0.0` rather than `127.0.0.1` — a silent, unverifiable assumption. The consumer must supply a `url` that is routable from the VM and bind the proxy on a VM-reachable interface. Also rejected: `defaultPolicy: "block"` (maps to `NetworkingMode::None` — no interface at all, so nothing could reach the proxy) and `allowedHosts`/`blockedHosts` alongside `proxy` (MXC does not forward host lists to an external proxy, and WSLC cannot enforce them itself).
>
