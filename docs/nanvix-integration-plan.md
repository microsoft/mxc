...
There is a need for a **micro-VM backend** that can execute various forms of code generated by agents — including Python, JavaScript, C, C++, and Rust applications — with full hardware isolation and rapid cold starts.

## Proposed Solution

Add a **NanVix micro-VM backend** directly into the existing `wxc-exec.exe` binary. When the JSON config specifies `"containment": "nanvix"`, the binary routes to a new `NanVixScriptRunner`. The runner spawns `nanvixd.exe` (the NanVix daemon), pipes the script via stdin, and captures stdout/stderr.
