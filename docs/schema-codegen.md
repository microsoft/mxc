...
published v0.9 contract. It remains renderable for verification; generation
does not make the stable artifact mutable.
...
It is assembled by exact contract adapters and has no JSON deserialization or
schema-generation surface.
...
3. provision additionally selects a backend-specific root by `containment`
   (Windows Sandbox, IsolationSession, or WSLC in v0.10; IsolationSession only
   in published v0.9).
