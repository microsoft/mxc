...
| --- | --- | --- |
| Rolling legacy | `src/core/wxc_common/src/wire.rs` | Differential parser/builder characterization and public SDK drift oracle |
| Exact `0.9.0-alpha` | `src/core/mxc_config_contract/src/dev/` | Authoritative closed development contract and versioned TypeScript oracle |

Neither family is hand-authored. Exact version dispatch is authoritative; the
rolling family remains temporarily as a differential migration oracle.

Published schemas under `schemas/stable/` are immutable release artifacts and
are not regenerated by `mxc_schema_gen` or `check-contract-codegen.js`.
...
retained for differential characterization, not production dispatch.

`src/core/mxc_config_contract/src/dev/` defines the exact mutable
`0.9.0-alpha` contract. Its one-shot and seven state-aware request roots are
