...
| `seatbelt.systemPowerAccess` | boolean | `false` | When `true`, allows sleep/wake notification registration and power assertions through `RootDomainUserClient`, `com.apple.PowerManagement.control`, and `com.apple.iokit.powerdxpc`. Off by default because `RootDomainUserClient` also exposes other host power-management operations. Requires schema `0.9.0-alpha` or later. |
