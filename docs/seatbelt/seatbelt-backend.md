...
| `network.ingress.hostLoopback` | Enforced on the container-to-host direction. `deny` emits `(deny network-outbound (remote ip "localhost:*"))`, which closes the host's loopback **and** the host's own non-loopback addresses. `allow` under a deny egress default emits the matching allow. The inbound half is not expressible in a Seatbelt profile, so `hostLoopback` must equal `default` or the config is **rejected**. An omitted `hostLoopback` is `deny`, not an inherit of `default` (see [#1032](https://github.com/microsoft/mxc/issues/1032)). Caveat: a `deny` also blocks the sandbox from connecting to its *own* loopback listeners. |
