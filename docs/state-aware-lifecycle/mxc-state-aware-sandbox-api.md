...
Each Config carries an optional backend-specific version literal.
IsolationSession uses `STATE_AWARE_VERSION` (`0.9.0-alpha`), while Windows
Sandbox and WSLC use `WINDOWS_SANDBOX_STATE_AWARE_VERSION` and
`WSLC_STATE_AWARE_VERSION` (both `0.10.0-alpha`). When omitted, the SDK supplies
the constant for the selected backend; an explicit value must match that
backend's registered state-aware contract. Other spellings are rejected, not
range-validated or negotiated. The emitted JSON envelope always contains the
...
as it is today for one-shot calls against `microvm` and `wslc`. Windows Sandbox and
WSLC require that opt-in; IsolationSession does not (§13).
...
interface ProvisionStateAwareRequest {
  ...
  isolationSession?: { provision?: { appId?: string } };
  wslc?: { provision?: { image?: string; imageTarPath?: string } };
}
...
interface NonProvisionStateAwareRequest {
  ...
  network?: NetworkConfig;                            // backend declares per-phase honor
  ui?: UiConfig;                                      // backend declares per-phase honor
  runtimeConfig?: RuntimeConfig;                      // WSLC exec only
}
...
|---|---|---|---|
| `version` | string | Yes | Exact backend-specific schema version. IsolationSession uses `0.9.0-alpha`; Windows Sandbox and WSLC use `0.10.0-alpha`. The SDK fills this field when the consumer Config omits it. |

Backend-specific top-level sections:

| Field | Type | Request roots | Description |
|---|---|---|---|
| `isolationSession` | object | IsolationSession provision in exact v0.9 | Optional `provision.appId`. |
| `wslc` | object | WSLC provision in exact v0.10 | Optional `provision.image` and `provision.imageTarPath`. |
...
### 7.2 Permanent backend sections

Backend-specific configuration uses the backend's permanent top-level section.
The exact request root determines whether that section and phase-specific
payload are legal:

- v0.9 IsolationSession provision may carry
  `isolationSession.provision.appId`;
- v0.10 WSLC provision may carry `wslc.provision.image` and
  `wslc.provision.imageTarPath`;
- Windows Sandbox provision has no backend-specific payload; its supported
  filesystem policy remains top-level.

Start, stop, and deprovision have no backend-specific configuration. WSLC exec
uses top-level `runtimeConfig.networkProxy`. There is no state-aware
`experimental` JSON wrapper; runtime experimental authorization is a separate
CLI/SDK option.
...
// `()`, so the envelope above has no backend-specific section and the dispatcher
// passes None:
backend.start(
...
| Code | Meaning |
|---|---|
| `malformed_request` | Structural request error: malformed JSON, missing required field, unknown or phase-inappropriate field, recursively unknown backend field, or invalid phase-specific shape |
...
duplicate/unknown fields, rejected nulls, and recursively unknown backend
fields. Structural failures surface as `malformed_request`
...
document; replacing or inserting the
...
```rust
StateAwareInput {
    common,             // no phase, containment, sandboxId, or backend section
...
enforced by the selected exact request root.

The construction boundary rejects contradictory common fields and one-shot
sections rather than silently discarding them. `normalize_state_aware` derives
a temporary phase/backend/ID context from the operation and invokes the shared
common conversion. It preserves policy presence and telemetry, infers the
existing containment context from the ID prefix without earlier ID validation,
and clears directional network defaults on later phases when no network policy
was supplied. It does not apply backend defaults or check host availability.

Backend binding and `validate_<phase>` hooks therefore receive only
structurally representable fields. They enforce semantic values, cross-field
...
`IsolationSession(Option<IsolationSessionProvisionConfig>)`, `WindowsSandbox`,
or `Wslc(Option<WslcProvisionConfig>)`; later variants each carry a required
`sandbox_id`, with no backend-specific configuration today. Exec process
settings stay in the common request.
...
checked against the selected backend, and passed as the `config` parameter
...
Both dispatch entry points accept `BoundStateAwareRequest<B>`; streaming accepts
only exec and passes `ExecStdio::Piped`, while the lifecycle path passes
`ExecStdio::Relayed`. Backend-specific piped-exec refusals remain unchanged.

Exact structural errors carry the full field path and source coordinates before
...
types declare the runtime configuration each phase consumes. Exact contracts own
JSON shape, and their adapters construct runtime values before binding. There is no
...
| MXC parser (Rust) | Exact registered version and closed request root; required phase fields; phase-inappropriate, unknown, and recursively unknown backend fields | `error.code: malformed_request`, `unsupported_phase`, `unsupported_containment` |
...
errors at compile time. The exact parser catches structural errors that escaped the
...
types separately define the JSON shape under the permanent top-level backend
section; adapters map their fields exhaustively to runtime values. Dispatch
does not require `Deserialize`. The
...
execution request. The SDK is responsible for splitting the consumer Config into top-level wire
fields (cross-cutting fields and `version`) and the permanent backend section;
Rust sees only the post-split shape.
...
wire-format containment value used for `provision`-phase routing and checked
binding. Extend the neutral operation and its mechanical binding helper for the
new backend, then use that helper in both engine dispatch paths. Compile-time errors catch
