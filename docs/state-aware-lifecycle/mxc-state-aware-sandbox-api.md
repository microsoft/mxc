...
| Rust executor (§9) | Exact phase contracts selected by version, phase, and provision containment; neutral operations; checked backend binding; and `StatefulSandboxBackend` dispatch. | `ScriptRunner` trait. Existing one-shot dispatch path. Existing backends function without modification. |
...
interface IsolationSessionProvisionConfig {
  ...
  // requires explicit acknowledgment. Filesystem policy is rejected (§10.3).
...
const provisionConfig: IsolationSessionProvisionConfig = {
  // IsolationSession cannot filter or deny the container network, so provision
  // requires the true-only acknowledgment.
  acknowledgeUnrestrictedNetwork: true,
};
...
```rust
// Exact adaptation preserves omitted network policy and places the positive
// acknowledgment in the typed provision configuration. After checked binding
// and backend validation, the dispatcher calls:
...
that phase. IsolationSession's required acknowledgment is backend-specific,
not a top-level network policy. The Rust struct receives what the wire's
`experimental.isolation_session.provision` block carries —
`{ "acknowledgeUnrestrictedNetwork": true, "appId": "PFN:Contoso.App_8wekyb3d8bbwe" }` — through exact adaptation and checked
...
`acknowledgeUnrestrictedNetwork: true` in the provision payload instead of
...
- **Compile-time enforcement at the SDK.** Each per-(backend, phase) Config (§6.1)
  ...
  *and* that the runtime currently honors. TypeScript rejects callers passing fields
  ...
  `acknowledgeUnrestrictedNetwork` field; filesystem/network/UI policy is rejected.
