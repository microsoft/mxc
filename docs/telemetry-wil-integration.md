...
| **Privacy Data Tags** | `TelemetryPrivacyDataTag(PDT_*)` | No equivalent | Not needed for current events |
| **Activity tracking** | `DEFINE_TELEMETRY_ACTIVITY` | Manual `Opcode` | Not needed for current events |

The "real gaps" (PDT, activity tracking, WIL error integration) are features MXC
does not currently use. If needed later, they can be added incrementally.
