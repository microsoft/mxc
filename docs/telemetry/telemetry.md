...
|---|---|---|
| `mxc.PolicyHash` | Every launch, after the effective request is resolved | `policy_hash`, `config_schema_version` |
| `mxc.SandboxIdentity` | After a successful state-aware phase | `phase` |
| `mxc.EnforcementDegraded` | ProcessContainer dispatch resolved below the preferred tier | `needs_dacl_augmentation`, `effective_enforcement_level`, `degradation_reasons`, `degradation_reason_count` |
...
| `mxc.SandboxTornDown` | Per-run resources released, once per handle | `status`, `firewall_rules_removed`, `firewall_removal_ok`, `bfs_removed`, `proxy_stopped`, `preserve_policy`, `container_released`, `skip_reason` |
| `mxc.ConfigRejected` | A request was refused before it could run | `reason`, `offending_field`, `phase` |
