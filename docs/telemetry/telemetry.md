...
| Enforcement degradation (M-ETW-2) | `MXC.EnforcementDegraded` | Reserved contract; not emitted by the PSEC-only ProcessContainer dispatcher | Not applicable to the current ProcessContainer or IsolationSession implementations, which have no tier ladder |
...
| `mxc.backend` | string | Backend identifier; ProcessContainer reports `processcontainer` |
...
diagnostic sinks. They exist so facts MXC already knows — a process exit code,
a teardown result, a rejected config — are *machine-readable*
...
| `mxc.EnforcementDegraded` | Reserved contract; current ProcessContainer dispatch does not emit it | `backend`, `identity`, `tier`, `needs_dacl_augmentation`, `effective_enforcement_level`, `degradation_reasons`, `degradation_reason_count` |
...
ProcessContainer no longer has a fallback tier ladder. Native PSEC either
supports the complete request or the launch fails closed, so
`mxc.EnforcementDegraded` is retained as an event contract but is not emitted by
...
* **`mxc.EnforcementDegraded` is a reserved contract.** The current PSEC-only
  dispatcher does not emit it because it either enforces the complete request
  or fails before launch.
...
| `mxc.EnforcementDegraded` | Reserved; not emitted | — | n/a (no tier model) | n/a |
| `mxc.NetworkPolicyApplied` | ✅ (PSEC) | — | — | — |
...
| Requirement | ProcessContainer (PSEC) | IsolationSession one-shot | IsolationSession state-aware |
|---|---|---|---|
| Process outcome (M-ETW-1) | ✅ | ✅ | ✅ (shared `create_process`) |
| Enforcement degradation (M-ETW-2) | Reserved; not emitted because there is no fallback ladder | n/a — no tier/fallback ladder exists for this backend | n/a |
| Policy hash (M-ETW-3) | ✅ | ✅ | ✅ |
| Network policy (M-ETW-4) | ✅ (`enforcement_mode: capabilities` — policy travels in the sandbox spec and the OS enforces it, so `firewall_rules_created` is honestly `0`) | n/a — MXC rejects network and proxy policy for this backend before provisioning | n/a |
| Sandbox teardown (M-ETW-5) | ✅ | ✅ | ✅ (`stop` and `deprovision` phases) |
| IsolationSession telemetry (M-ETW-6) | n/a | ✅ Applicable lifecycle events use `Microsoft.MXC`; no separate OS provider is assumed | ✅ Same provider path |
...
| Enforcement degradation (M-ETW-2) | Not applicable to these backends: neither current implementation has a tier/fallback model. | `mxc.EnforcementDegraded` remains reserved but is not emitted by the current dispatcher. | No tier join is expected. |
...
| Network policy (M-ETW-4) | Not applicable to `isolation_session`: MXC rejects its network and proxy policy before OS provisioning. | `mxc.NetworkPolicyApplied` covers OS-enforced PSEC network setup. | This row changes only if the separate M1 network-proxy requirement is implemented. |
