...
`seatbelt_net_ingress_allow_loopback_deny.json` joined the corpus. It rose by a
further twelve when the `{bwrap,lxc,seatbelt}_env_09_*.json` fixtures for the
0.9 `inheritDefaultEnv` contract joined the corpus. Both parsers accept each of
those and build identical runtime models, so they add no shared rejection. Both
retain no exact-looser acceptance and no accepted-model mismatch. Assertion
failures list the shared-rejection files so future platform-specific movement is
attributable rather than represented only by aggregate counts.
...
## Directional networking cutover

This atomic cutover builds on the exact-contract baseline at `16ed3c81`.
