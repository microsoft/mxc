...
### 3. “Why reject unknown fields? Isn’t ignoring them more forward-compatible?”
...
### 7. “If SDKs target a major line, why does the wire need exact minor versions?”
...
### 9. “Do SDK callers still end up using serialized JSON under the hood?”

Some SDK paths may serialize an exact JSON contract internally; other paths
may use a typed private FFI request. That transport is an SDK implementation
detail rather than part of the high-level caller contract.

The high-level caller supplies typed policy intent, and the SDK owns both the
package-selected exact target and any conversion needed to reach the runtime.
When JSON crosses the native boundary, it conforms to the selected exact
contract. Raw configuration APIs expose that JSON and version choice
directly.

State-aware JSON transport may remain while typed FFI evolves independently.
Changing the internal transport does not change the SDK's public compatibility
promise.

### 10. “Do raw JSON users rewrite their config for every minor release?”
...
### 11. “What counts as a breaking change within a major line?”
...
### 12. “Can a minor version add a required field?”
...
### 13. “What happens if the SDK is newer than the native runtime?”
...
### 14. “Is the entire v1 model implemented today?”
...
### 15. “Won’t one Rust type per version create endless maintenance?”
...
### 16. “Are we going to have `if version >= ...` checks throughout the runtime?”
...
### 17. “If a field exists in one-shot, can I use it during state-aware provision?”
...
### 18. “Why can’t every state-aware phase accept the full policy?”
...
### 19. “Why does `1.0.0` come from v0.9 instead of v0.10?”
...
### 20. “Are we throwing away the v0.10 work?”
...
### 21. “Why are Windows Sandbox, Hyperlight, and MicroVM in `1.1.0`?”
...
### 22. “Why not put experimental structures into `1.0.0`?”
...
### 23. “Why change the old aliases at the v1 boundary?”
...
### 24. “Why do we need both Rust deserialization and JSON Schema tests?”
...
### 25. “Why commit generated schemas and TypeScript wire files?”
...
### 26. “Can we fix bugs in a published contract?”
...
### 27. “What protects a published contract from accidental change?”
...
### 28. “What is the one question I should ask before opening a PR?”
