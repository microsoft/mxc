...
Rust, .NET, and Node high-level one-shot policy and typed lifecycle APIs are
version-free. Each SDK major owns its exact contract target; the v1 SDK
currently emits exact stable `1.0.0`.
...
shapes. Windows Sandbox lifecycle is available only through the raw exact
`1.1.0-alpha` surface; it is intentionally absent from all high-level v1
lifecycle APIs.
...
registered for its declared version. High-level one-shot and typed lifecycle
builders target the SDK-owned stable contract instead. Corpus validation
selects the exact registered schema from each raw document's `version`.
