...
The state-aware daemon listens on an OS-assigned localhost TCP port (recorded in its `daemon.json` record). Clients (`wxc-exec` phase processes) connect and issue a verb on a single line (`<VERB> <nonce>\n`); there is **no** binary preamble on this channel (the 8-byte preamble is on the daemon↔guest control channel — see below):
...
1. Terminates **exactly** the Windows Sandbox host processes this run
   provably launched, identified by `(pid, creation_time)` ownership proof
   (never an image-wide `taskkill /IM`), so a foreign or manually-opened
   sandbox is never disturbed. Each target is opened by PID and its creation
   time re-verified on the same handle before `TerminateProcess`, defeating
   PID reuse.
