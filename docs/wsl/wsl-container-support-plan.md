...
- **WSLC SDK available:** The SDK (v2.9.9) is consumed from the
...
Rust FFI declarations for the WSLC SDK C API (v2.9.9). Generated by bindgen from
`wslcsdk.h` into `wslcsdk_sys.rs` (see
[`wslc-sdk-bindings.md`](wslc-sdk-bindings.md)); `wslc_bindings.rs` is a thin
facade over it. Key bindings (actual API names from the SDK):
...
- Windows 10 version 1903 (build 18362.1049) or later on x64, or Windows 10
  version 2004 (build 19041) or later on ARM64 — checked by
  `wslc_common::host_requirements` before the backend is reported or launched
