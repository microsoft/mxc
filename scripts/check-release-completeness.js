...
//
// Release-completeness gate. The on-disk platform and companion package
// directories are the canonical source of truth. The gate asserts:
//   - the expected set is non-empty (a missing/corrupt set can't pass silently);
//   - the meta package pins EXACTLY the platform packages at their versions;
//   - companion packages peer-depend on the exact matching SDK version;
//   - every expected binary-package tarball was produced (no MISSING); and
...
//
//   META_PKG=<sdk/node/package.json> PLATFORM_PACKAGES_DIR=<sdk/node/platform-packages> \
//   COMPANION_PACKAGES_DIR=<sdk/node/companion-packages> \
...
/**
 * @param {{ metaPkgPath: string, platformPackagesDir: string,
 *           companionPackagesDir?: string, tarballDir: string,
...
function checkReleaseCompleteness({
  ...
  const meta = JSON.parse(readFileSync(metaPkgPath, "utf8"));
  const opt = meta.optionalDependencies || {};

  // Expected tarballs derived from the on-disk platform and companion packages.
  ...
  for (const [kind, packagesDir] of [
    ...
    if (!packagesDir || !existsSync(packagesDir)) continue;
    const dirs = readdirSync(packagesDir);
    for (const d of dirs) {
      const pj = join(packagesDir, d, "package.json");
      ...
      if (kind === "platform") {
        platformNames.add(pkg.name);
        if (opt[pkg.name] !== pkg.version) {
          result.pinIssues.push(
            `${pkg.name}: meta pins "${opt[pkg.name]}" but the package is "${pkg.version}"`,
          );
        }
      } else if (pkg.peerDependencies?.["@microsoft/mxc-sdk"] !== meta.version) {
        result.pinIssues.push(
          `${pkg.name}: SDK peer is "${pkg.peerDependencies?.["@microsoft/mxc-sdk"]}" ` +
            `but expected exact "${meta.version}"`,
        );
      }
      for (const f of payloadFiles(pj, { readFileSync })) {
        if (!existsSync(join(packagesDir, d, f))) {
  ...
  for (const name of Object.keys(opt)) {
    if (name.startsWith("@microsoft/mxc-sdk-") && !platformNames.has(name)) {
...
if (require.main === module) {
  ...
  const companionPackagesDir = process.env.COMPANION_PACKAGES_DIR;
  const tarballDir = process.env.TARBALL_DIR;
  if (!metaPkgPath || !platformPackagesDir || !companionPackagesDir || !tarballDir) {
    console.error(
      "check-release-completeness: META_PKG, PLATFORM_PACKAGES_DIR, " +
        "COMPANION_PACKAGES_DIR and TARBALL_DIR are required",
    );
    process.exit(2);
  }
  ...
  console.log(
    `Release completeness OK: ${r.expected} binary package(s), compatible and packed, no strays.`,
  );
}
