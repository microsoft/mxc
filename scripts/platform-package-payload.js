...
function payloadFiles(manifestPath, { readFileSync = fs.readFileSync } = {}) {
  ...
  if (!Array.isArray(pkg.files) || pkg.files.length === 0) {
    throw new Error(`${manifestPath}: files must be a non-empty array`);
  }
  if (pkg.files.some((f) => typeof f !== "string" || f.length === 0)) {
    throw new Error(`${manifestPath}: files entries must be non-empty strings`);
  }
  const payload = pkg.files.filter((f) => !NON_ARTIFACT_FILES.has(f));
  if (payload.length === 0) {
    throw new Error(`${manifestPath}: files must include at least one build artifact`);
  }
  return payload;
}
