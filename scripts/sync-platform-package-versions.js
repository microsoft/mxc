...
function syncPlatformPackageVersions({ repoRoot, check = false }) {
  ...
  const pending = [];
  for (const packagesDir of [platformPackagesDir, companionPackagesDir]) {
...
if (require.main === module) {
  ...
  if (check) {
    if (result.drifted.length > 0) {
      ...
      console.error(
        `\n${result.drifted.length} SDK package version(s) drifted from ` +
    ...
    console.log(
      `SDK package versions OK: ${result.checked} package(s) match ` +
        `meta version ${result.metaVersion}`,
    );
  } else {
