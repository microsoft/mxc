...
// Learning-mode capabilities are reserved: they are injected by the runner /
// --audit flow, never requested from a shipped config.
...
// macOS/seatbelt). Requires the built SDK at `sdk/node/dist`.
...
async function checkSdkEmission(failures, { required }) {
  const distEntry = join(repoRoot, "sdk", "node", "dist", "index.js");
  if (!existsSync(distEntry)) {
    if (required) {
      failures.push(
        `SDK emission check is required but ${distEntry} is missing — run \`npm run build\` in sdk/node first.`
      );
    } else {
      console.log("SDK emission: skipped (sdk/node/dist not built).");
    }
    return;
  }
...
async function main() {
  // `--sdk-emission=required` makes a missing sdk/node/dist a failure (used by the
