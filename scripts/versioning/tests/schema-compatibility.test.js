...
test("an added property that matched patternProperties needs review", () => {
  for (const additionalProperties of [false, { type: "number" }]) {
    const previous = {
      type: "object",
    ...
    const breaks = detectBreaking(previous, next);
    assert.ok(
      breaks.some((entry) => entry.includes("overlaps previous patternProperties")),
      breaks.join("\n")
    );
  }
...
test("an identical unresolved reference on both sides is still reported", () => {
  ...
  for (const keyword of ["allOf", "oneOf", "anyOf"]) {
    const composed = {
      [keyword]: [{ $ref: "https://example.com/x.json" }],
    };
    assert.ok(
      detectBreaking(composed, composed).length > 0,
      `${keyword} silently matched an unresolved reference`
    );
  }
});
