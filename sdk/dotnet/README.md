...
Package-consumer integration tests live under `tests/integration`. They have no
project reference to the SDK and restore only the supplied `.nupkg`, then verify
the managed assembly, native library, current-RID assets, platform probe, and
supported backend execution:

```powershell
sdk\dotnet\tests\integration\run.ps1 `
  -PackagePath output\packages\Microsoft.Mxc.Sdk.<version>.nupkg
```

Use `-SkipBackendTests` when the current host is not prepared to launch a
sandbox. Linux CI runs the backend tests under `sudo`; Windows ProcessContainer
execution requires `MXC_E2E_HOST_PREPPED=1`.
...
In CI, the Rust build stage publishes both the production native unit and a
separate debug `test-support` library; the .NET jobs consume those artifacts
and do not invoke Cargo. Local `dotnet test` builds the isolated test-support
library when no prebuilt path is supplied.
