...
- `getPlatformSupport()` now reads the native `mxc_ffi` platform-support
  surface instead of reimplementing host probes in JavaScript.
- `getAvailableBackends()` exposes the separate native host-capability probe,
  including effective tiers, optional capabilities, and capability warnings.
