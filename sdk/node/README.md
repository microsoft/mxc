...
>
> **NanVix is an explicit companion install.** The large MicroVM payload is
> not included in the automatic win32-x64 package. Windows x64 consumers using
> `containment: "microvm"` must run
> `npm install @microsoft/mxc-sdk @microsoft/mxc-sdk-nanvix-win32-x64`.
> The SDK selects the companion package's co-located executor for MicroVM
> requests and reports this command if the package is absent.
>
...
| `microvm` | `microvm` | Windows x64 | Experimental | [`docs/nanvix-microvm/nanvix.md`](https://github.com/microsoft/mxc/blob/main/docs/nanvix-microvm/nanvix.md) — requires explicit `@microsoft/mxc-sdk-nanvix-win32-x64` installation |
...
| `NanVix MicroVM runtime not found` | A MicroVM request was made without the explicit Windows x64 companion package. | Run `npm install @microsoft/mxc-sdk @microsoft/mxc-sdk-nanvix-win32-x64`, or provide a co-located development payload through `MXC_BIN_DIR`. |
