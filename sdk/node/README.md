...
| `0.9.0-alpha` | Stable (includes IsolationSession one-shot and state-aware lifecycle) | [`schemas/stable/mxc-config.schema.0.9.0-alpha.json`](https://github.com/microsoft/mxc/blob/main/schemas/stable/mxc-config.schema.0.9.0-alpha.json) |
...
WSLC, MicroVM, and Hyperlight require `0.10.0-alpha`; Seatbelt requires
`0.7.0-alpha` or later.

> **Stable schemas document only the non-experimental surface.** Experimental backends (`windows_sandbox`, `wslc`, `microvm`, `hyperlight`) and their permanent backend sections are defined by the mutable development contract. IsolationSession and its state-aware lifecycle are part of exact v0.9 and do not require `--experimental`. Production executors dispatch through the exact contract selected by the declared version, whose adapter normalizes it into the private runtime input.
...
The default `processcontainer`, `bubblewrap`, `lxc`, `seatbelt`, and `isolation_session` backends work without an experimental opt-in. **Experimental backends** (`windows_sandbox`, `wslc`, `microvm`, `hyperlight`) require `{ experimental: true }` in `SandboxSpawnOptions` when you spawn — see [Choosing a Backend](#choosing-a-backend).
...
| `wslc` | (concrete only) | Windows | `0.10.0-alpha` | Experimental | [`docs/wsl/wsl-container-getting-started.md`](https://github.com/microsoft/mxc/blob/main/docs/wsl/wsl-container-getting-started.md) |
...
> **Backend support:** the state-aware lifecycle is currently implemented for `isolation_session`, `windows_sandbox`, and `wslc` (all Windows-only). IsolationSession does not require an experimental opt-in; Windows Sandbox and WSLC do. The one-shot spawn APIs (`spawnSandbox` / `spawnSandboxFromConfig`) are the supported path for every other backend.
...
Windows Sandbox and WSLC requests default to development `0.10.0-alpha`. See
...
| `'<x>' containment requires experimental mode` | A `windows_sandbox` / `wslc` / `microvm` / `hyperlight` backend was selected without the flag. | Pass `{ experimental: true }` in `SandboxSpawnOptions`. |
...
| `Policy version '<x>' is not a registered schema contract` / `Unsupported contract version` | The declaration is not registered, even if it falls between supported versions (for example, `0.6.1-alpha`). | Use an exact version from [Compatibility](#compatibility); IsolationSession state-aware requests use `0.9.0-alpha`, while Windows Sandbox and WSLC use `0.10.0-alpha`. |
| `Schema <x> does not support containment '<backend>'` | The selected backend was introduced after the declared schema version. | Use the backend's minimum version from [Choosing a Backend](#choosing-a-backend). Seatbelt requires `0.7.0-alpha`; IsolationSession requires `0.9.0-alpha`; Windows Sandbox, WSLC, MicroVM, and Hyperlight require `0.10.0-alpha`. |
