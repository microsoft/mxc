...
export interface IsolationSession {
  ...
  /**
   * Deprecated flat spelling of the Entra cloud-agent user bundle, retained so it is *rejected* rather than silently ignored. It is not honored on either surface: the one-shot path has no Entra mode, and a state-aware request must nest the bundle under the phase it applies to. Supply it as `experimental.isolation_session.provision.user`, and again at `experimental.isolation_session.start.user` to re-present the token.
   */
