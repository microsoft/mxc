...
import { findWxcExecutable, findLxcExecutable, findSeatbeltExecutable, findNanvixExecutable, getPlatformSupport, getPlatformPackageName, getExecutableBinaryName, isSupportedPlatformTuple, NANVIX_PACKAGE_NAME, verifyExecutable } from './platform.js';
...
function missingBinaryMessage(binary: string, extraHint?: string): string {
  ...
  if (!isSupportedPlatformTuple()) {
    parts.push(
      ...
      `options.executablePath, or set both skipPlatformCheck and the MXC_BIN_DIR`,
      `environment variable (to a directory whose <arch> subdirectory holds a`,
      `compatible binary).`,
    );
  } else {
...
export function resolveBinaryAndCommonArgs(
  ...
  runtime: 'default' | 'nanvix' = 'default',
  ...
  if (platform === 'linux') {
    ...
    const p = runtime === 'nanvix' ? findNanvixExecutable() : findWxcExecutable();
    if (!p) {
      if (runtime === 'nanvix') {
        throw new Error(
          `NanVix MicroVM runtime not found. Install the explicit companion package ` +
            `"${NANVIX_PACKAGE_NAME}" alongside "@microsoft/mxc-sdk" ` +
            `(npm install @microsoft/mxc-sdk ${NANVIX_PACKAGE_NAME}), or set the ` +
            `MXC_BIN_DIR environment variable to a directory whose x64 subdirectory ` +
            `contains wxc-exec.exe and the NanVix payload.`,
        );
      }
      throw new Error(
...
export function resolveExecutableAndArgs(
  ...
  const runtime = effectiveContainment === 'microvm' ? 'nanvix' : 'default';
  const resolved = resolveBinaryAndCommonArgs(JSON.stringify(config), options, runtime);
