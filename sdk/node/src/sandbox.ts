...
export function createConfigFromPolicy(
    ...
    // Presence matters for backends that cannot enforce UI policy. Do not
    // synthesize a UI block when the caller did not request one.
...
export function spawnSandboxProcess(
  ...
  return spawnSandbox(script, policy, options, workingDirectory, containerName);
}
