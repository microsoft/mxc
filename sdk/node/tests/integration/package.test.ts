...
// Files allowed to sit beside the binaries in a platform package (its tracked
// metadata — the manifest, readme, and license — everything else must be a
// shipped binary from the manifest's `files` allowlist).
const ALLOWED_NON_BINARY_FILES = ['package.json', 'README.md', 'LICENSE.md'];

describe('per-platform package binaries', () => {
  // The native executor and sandbox helpers ship in the host's per-platform
  // package (@microsoft/mxc-sdk-<os>-<arch>) at its root, not in a bin/<arch>
  // subdirectory of the meta package.
