...
import {
  ...
  _setNanvixPackageDir,
  _setDevMode,
  _setHostId,
  _setSdkRequire,
  _setSdkPackageRoot,
  _validatePlatformPackageDir,
  _validateNanvixPackageDir,
  NANVIX_PACKAGE_NAME,
...
describe('per-platform binary package discovery', () => {
  ...
  afterEach(() => {
    ...
    _setNanvixPackageDir(undefined);
    _setDevMode(null);
    _setHostId(null);
  ...
  it('findNanvixExecutable uses the explicit companion package, not the platform package', () => {
    _setHostId({ platform: 'win32', arch: 'x64' });
    const platformDir = makeTempDir();
    stageBinary(platformDir, 'wxc-exec.exe');
    _setPlatformPackageDir(platformDir);

    const nanvixDir = makeTempDir();
    const expected = stageBinary(nanvixDir, 'wxc-exec.exe');
    _setNanvixPackageDir(nanvixDir);

    assert.strictEqual(findNanvixExecutable(), expected);
  });

  it('findNanvixExecutable fails closed when the companion package is absent', () => {
    _setHostId({ platform: 'win32', arch: 'x64' });
    const platformDir = makeTempDir();
    stageBinary(platformDir, 'wxc-exec.exe');
    _setPlatformPackageDir(platformDir);
    _setNanvixPackageDir(null);

    assert.strictEqual(findNanvixExecutable(), null);
  });
  ...
  it('_validateNanvixPackageDir requires the companion identity and SDK version', () => {
    const dir = makeTempDir();
    const pkgPath = path.join(dir, 'package.json');

    fs.writeFileSync(
      pkgPath,
      JSON.stringify({ name: NANVIX_PACKAGE_NAME, version: '0.7.0' }),
    );
    assert.strictEqual(_validateNanvixPackageDir(pkgPath), dir);

    fs.writeFileSync(
      pkgPath,
      JSON.stringify({ name: getPlatformPackageName(), version: '0.7.0' }),
    );
    assert.strictEqual(_validateNanvixPackageDir(pkgPath), null);

    fs.writeFileSync(
      pkgPath,
      JSON.stringify({ name: NANVIX_PACKAGE_NAME, version: '9.9.9' }),
    );
    assert.strictEqual(_validateNanvixPackageDir(pkgPath), null);
  });
  ...
  // Arbitrary version used to stamp BOTH the fake meta package.json and the fake
  // sibling platform package in these hermetic fixtures. The identity/version
  // guard compares the two against each other (via the injected _setSdkPackageRoot
  // seam), so this need NOT track the repo's real SDK version — only be internally
  // consistent within each fixture.
  const FIXTURE_VERSION = '0.7.0';
  ...
  function fakeInstall(opts: { siblingName?: string; siblingVersion?: string; withBinary?: boolean } = {}) {
    ...
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ name: '@microsoft/mxc-sdk', version: FIXTURE_VERSION }));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: opts.siblingName ?? getPlatformPackageName(), version: opts.siblingVersion ?? FIXTURE_VERSION }),
    );
  ...
  it('P1-2: dev mode prefers a locally-staged binary over the installed package', () => {
    ...
    _setHostId({ platform: 'win32', arch: os.arch() === 'arm64' ? 'arm64' : 'x64' });
    const tmp = makeTempDir();
    const root = path.join(tmp, 'repo', 'sdk', 'node');
    fs.mkdirSync(root, { recursive: true });
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ name: '@microsoft/mxc-sdk', version: FIXTURE_VERSION }));
