...
class FakeDriver implements NativeLifecycleDriver {
  ...
  private completeDeferredWait:
    ...
    | undefined;
  private completeDeferredFree: (() => void) | undefined;
  ...
  resolveFree(): void {
    this.completeDeferredFree?.();
    this.completeDeferredFree = undefined;
  }
...
describe('native sandbox process', () => {
  ...
  it('defers stream failure finalization until native wait completes', async () => {
    const driver = new FakeDriver();
    driver.deferWait = true;
    const scheduler = new ManualScheduler();
    const proc = createMxcSandboxProcess(driver, undefined, scheduler);
    ...
    await assert.rejects(wait, /stdout failed/);
  ...
  it('captures stream failures emitted while native cleanup is pending', async () => {
    const driver = new FakeDriver();
    driver.deferWait = true;
    driver.deferFree = true;
    const scheduler = new ManualScheduler();
    const proc = createMxcSandboxProcess(driver, undefined, scheduler);
    const wait = proc.waitAsync();
    driver.complete(5);
    ...
    driver.standardOutput.emit('error', new Error('late stdout failure'));
    driver.resolveFree();

    await assert.rejects(wait, /late stdout failure/);
    assert.strictEqual(driver.freeCount, 1);
  });
});
