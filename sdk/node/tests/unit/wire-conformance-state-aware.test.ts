...
//
// `filesystem`, `containerId`, and `telemetry` are lifted top-level wire fields
// (like `network`): phase configs surface them publicly but they map to the
// envelope top level, not under `experimental.<backend>.<phase>`. Listing them
// here keeps the backend-key set limited to genuinely per-phase wire fields.
// `containerId` is lifted for every backend -- `CROSS_CUTTING_FIELDS` in
// `state-aware-helper.ts` moves it to the envelope top level, so it never
// reaches a per-phase object.
type LiftedPhaseKey =
    | 'version'
    | 'process'
    | 'network'
