 ...
 * (`wxc-exec`) attaches a per-PID ETW collector to the sandboxed
 * process and writes one NDJSON line per captured denial to its own
 * stderr, prefixed with the ASCII Record Separator byte (0x1E):
 *
 * ```text
 ...
 * segment as JSON.
 *
 * Scope: filesystem/registry denials only. Network/WFP denials live
 * in different ETW providers with a different shape and are not yet
 * captured; the `DeniedResource` discriminated union is designed so
 * adding a `kind: "network"` variant later is additive.
 */
...
export interface DenialStreamSummary {
   ...
   * activate it — e.g. the `MxcLearningModeShim` service isn't installed
 ...
 *
 * These filters remove the bulk of the AppContainer / BaseContainer
 * "background hum" — access checks the kernel records for every
 * sandboxed process regardless of what the workload is actually
 * doing (locale registry probes, loader DLL searches in
 * `C:\Windows\System32`, etc.). Callers who need the full unfiltered
 * stream (e.g. for diagnostics or building a noise allow-list) can
 * pass `filters: 'none'`.
 */
 ...
 * config.process!.commandLine = 'type C:\\path\\to\\file.txt';
 *
