 ...
 *
 * Windows-only. captureDenials itself is Windows-only today, so
 * this matches the feature scope. The module type-checks and
 ...
 * client disconnects.
 *
 * Throws on non-Windows hosts (Unix doesn't have Windows named
 * pipes, and captureDenials is Windows-only anyway).
 */
export function createDenialPipeServer(): DenialPipeServer {
  if (os.platform() !== 'win32') {
    throw new Error(
      'createDenialPipeServer: Windows named pipes only. ' +
        'captureDenials is currently Windows-only; this transport is too.',
    );
  }
