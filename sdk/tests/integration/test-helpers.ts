 ...
 *   2. the monorepo dev layout `sdk/platform-packages/<os>-<arch>`.
 *
...
export function getPlatformPackageDir(): string {
  ...
  throw new Error(
    `Platform package ${name} is not installed and no dev layout was found ` +
      `(sdk/platform-packages/${tuple}). The integration suite needs the native ` +
 ...
 * (`wxc-test-proxy.exe` / `linux-test-proxy`). These are intentionally NOT
...
// proxies (wxc-test-proxy.exe / linux-test-proxy) or the diagnostic console,
...
/** Locate linux-test-proxy in the test-proxy source dir (not shipped in the package). */
function findLinuxTestProxyBinary(): string {
  const binDir = getTestBinDir();
  const proxyPath = binDir ? path.join(binDir, 'linux-test-proxy') : null;
  if (proxyPath && fs.existsSync(proxyPath)) {
    return proxyPath;
  }
  throw new Error(
    'linux-test-proxy not found. It is excluded from the shipped platform ' +
      'package (#512); set MXC_TEST_PROXY_DIR or build the Rust binaries locally.',
  );
}
