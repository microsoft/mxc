...
//! other callers) to `mxc-learning-mode-shim` for per-PID denial capture.
//!
//! Phase 3 split:
...
impl CollectorHandle {
    ...
    pub fn stop_and_drain(mut self) -> (Vec<DenialEvent>, bool) {
        ...
        if kernel_lost_any {
            eprintln!(
                "[learning_mode_windows] ETW kernel buffer loss: events_lost={}, buffers_lost={} \
...
fn stop_session_by_name(name: &str) -> SessionStopStats {
    ...
    if status != WIN32_ERROR(0) {
        eprintln!(
            "[learning_mode_windows] ControlTraceW(STOP) for {name} returned {:#X}",
            status.0
        );
        ...
        let worker = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if status != WIN32_ERROR(0) && status.0 != 1223 {
                    eprintln!(
                        "[learning_mode_windows] ProcessTrace returned Win32 {:#X}",
                        status.0
                    );
                }
            })
...
/// Opens a privileged ETW session via the `mxc-learning-mode-shim` service.
///
