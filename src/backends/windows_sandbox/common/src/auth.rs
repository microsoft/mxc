...
//! isn't the daemon that launched this VM.
//!
...
/// order**.
///
/// Accept-FIFO order is not guaranteed to match the host's connect
/// order: on Hyper-V vNIC paths (or under any kernel / firewall
/// scheduling that delivers a later listen-queue entry before an earlier
/// one under unrelated load) the ordering can differ. Pairing by accept
/// order would then mispair the child's streams — e.g. the child's
/// "stdout" paired with the host's "stdin" socket, so bytes the child
/// wrote vanish into a reader nobody consults while the host's stdout
/// reader blocks forever on a socket the guest never writes to.
///
/// One byte after the nonce on every TCP connection (boot and reconnect)
/// is a `ChannelRole`. The guest reads it under the same auth
...
/// applied (and be owner-verified); the file inherits it.
///
/// Any pre-existing `nonce.bin` is **removed first** rather than truncated in
/// place: `std::fs::write` truncates an existing file but preserves its
/// explicit ACEs, so a cross-user attacker who pre-created the file (in the
/// narrow window before the parent dir is locked) with a readable ACE could
/// otherwise read the nonce we write. Deleting and recreating yields a fresh
/// file that inherits only the parent's owner-only DACL.
