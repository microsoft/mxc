...
/// the [`CLIENT_READ_TIMEOUT`] elapsed. The legitimate verbs
...
/// stacks indefinitely, crowding out real EXEC/STOP traffic. The
...
/// permit before being dropped. A non-blocking `try_acquire_owned()` would
/// instant-drop the 33rd concurrent connection, so a legitimate burst from
/// `make -j` or a CI batch that briefly exceeded the cap would surface as a
/// spurious backend error with no client-side retry. Permits are released in
/// milliseconds for normal traffic
...
/// cannot steal a per-exec stream; see the
...
pub async fn run(
    ...
    // Pre-auth concurrency cap. Bounds the number of unauthenticated
...
///
/// `permit` is the pre-auth concurrency token. For PING/STOP it is held for the
/// whole short handler. For EXEC it is passed into [`handle_exec`], which holds
/// it through admission (framed-request read + single-flight acquire) and
/// releases it the instant the exec is admitted — so a long-running command
/// does not pin a pre-auth slot, yet a flood of authenticated-but-unadmitted
/// EXECs cannot accumulate unboundedly.
async fn handle_client(
    ...
    // Cap the pre-auth read at MAX_AUTH_LINE_BYTES. Without this
    ...
    if verb == IPC_PING {
        ...
        // Hold the pre-auth concurrency slot through the EXEC admission
        // decision (framed `ExecStart` read + single-flight `try_lock`): a
        // client must not be able to fire authenticated EXEC requests faster
        // than they admit and queue an unbounded number of tasks + sockets
        // ahead of admission, exhausting daemon FDs/memory. The permit is
        // released inside `handle_exec` the moment the exec is admitted — a
        // long-running command is then bounded by the single-flight guest
        // mutex (only one exec runs at a time), not by a pinned pre-auth slot.
...
async fn handle_exec(
    ...
    // map the slot state to a typed AdmissionDecision. The
    ...
    // Admitted and acknowledged: release the pre-auth slot now. From here the
    // single-flight guest mutex (we hold `slot`) bounds concurrency to one
    // in-flight exec, so a long-running command need not occupy a
    // pre-auth slot.
    drop(permit);
    ...
    match stream_exec_on_guest(&mut conn, &exec_id, &req, &mut reader, &mut writer).await {
        Ok(outcome) => {
            ...
            // never races the slot-release window.
            let reconnect =
