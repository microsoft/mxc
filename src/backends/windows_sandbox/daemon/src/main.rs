...
async fn main() -> Result<()> {
    ...
    // Secure the state-aware record root (owner-verify + owner-only DACL)
    // BEFORE reading any record out of it, so a `record.json` / `daemon.json`
    // an attacker pre-planted under a shared-temp root is refused rather than
    // trusted. Fails closed on a foreign-owned root.
    ...
    match control_plane::classify_startup(prior.as_ref(), &current_vm) {
        ...
        control_plane::StartupAction::ReclaimOrphan { proof } => {
            ...
            // kill set. Seeding with the live snapshot instead would promote
            // observed processes into "proof" and risk killing a foreign VM.
    ...
    // Run the serving lifetime in its own task and JOIN it so a panic in
    // `serve` (or anything it awaits) is captured as a `JoinError` and still
    // flows into the guaranteed ownership-scoped teardown below, instead of
    // unwinding straight out of `main` and leaking the VM. Teardown reads the
    // shared `ownership` Arc, which a panic leaves intact.
