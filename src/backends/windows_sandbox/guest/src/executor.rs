                ...
                DecodeResult::Message { message, consumed } => {
                    ...
                    match message {
                        ControlMessage::Exec(req) => {
                            // Do NOT log the raw `script_code`.
...
async fn execute_script(
    ...
    let mut stdout_task = tokio::spawn(async move {
        if let (mut tcp, Some(mut child_out)) = (stdout_stream, child_stdout) {
            ...
            // window where the host observes EOF before the payload.
            // NODELAY is the right default for a small, latency-sensitive
            // command-output stream (the role-tag handshake handles socket
            // pairing; see `ChannelRole`).
            let _ = tcp.set_nodelay(true);
