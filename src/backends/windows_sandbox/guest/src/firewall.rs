...
///
/// The allow rule is scoped to the host's listener port: allowing all outbound
/// TCP to `host_ip` regardless of source port would let untrusted workloads
/// reach arbitrary host services on the same IP, defeating the "block all
/// network" isolation.
