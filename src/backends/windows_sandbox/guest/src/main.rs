...
async fn main() -> Result<()> {
    ...
    // Step 0a: read & immediately delete the per-launch authentication nonce.
    // A missing / malformed file is fatal: without it we cannot authenticate
    // host connections (the only alternatives are an insecure accept-by-order or
    // a deadlock).
    ...
    // Step 0b: pre-authorize in Windows Firewall before binding so the host's
    // inbound connection doesn't trigger an interactive "allow this app?" prompt
    // that blocks unattended runs. Best-effort — a netsh hiccup is non-fatal.
