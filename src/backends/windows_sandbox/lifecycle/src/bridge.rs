...
/// (constant-time compare) and drops the connection on mismatch — closing the
/// **cross-user** hijack window that an unauthenticated accept-by-order scheme
/// would leave open. See [`auth`] for the full threat model
...
pub async fn connect_to_guest(
    ...
    let connect = |role: ChannelRole| async move {
        ...
        // tag lets the guest assign each accepted socket by identity rather
        // than by accept order, which can be reordered by the Hyper-V vNIC
        // accept queue (see ChannelRole docs and `accept_one_authed` on the
...
pub async fn execute_on_guest(
    ...
    // Forward stdin to the guest CONCURRENTLY with draining stdout/stderr and
    // reading the control channel (below). Writing the whole stdin buffer
    // before reading any output deadlocks when the guest emits enough output to
    // fill the stdout TCP window before it finishes consuming stdin: the host
    // blocks on the stdin write while the guest blocks on its stdout write, and
    // with `timeout_ms == 0` that hangs forever. Pumping all channels at once
    // (matching the state-aware streaming path) removes the deadlock.
    //
    // A stdin-forwarding error is best-effort, NOT fatal: a command that
    // ignores stdin and exits closes its stdin socket, so `write_all` can fail
    // with BrokenPipe on a perfectly successful run. The captured output + exit
    // code are authoritative; a stdin error is logged and otherwise ignored.
    let stdin_task = {
    ...
    // Read stdout/stderr to EOF, preserving any bytes received even if the read
    // later errors. Each chunk is classified with the same [`classify_data_read`]
    // semantics as the streaming path: a clean EOF (and a reset *before any
    // bytes*, which is how the guest closing an empty stream can surface) ends
    // the drain with no error; a reset *after* bytes (or any other error) is a
    // genuine transport failure that must not be silently swallowed. Each task
    // returns (buf, Option<error>) so partial output is never discarded.
    let stdout_task = {
...
/// threat is identical at boot and at reconnect; see the
        ...
        loop {
            match decode_message(&buf).context("decode StreamsReady")? {
                DecodeResult::Message {
                    ...
                    // Reconnect the three data streams CONCURRENTLY: each
                    // connect+auth handshake is bounded by RECONNECT_TIMEOUT_SECS
                    // and they are independent, so `try_join!` keeps the exec
                    // completion path to a single timeout window (vs. up to 3×
                    // serially) and fails fast if any one stream fails.
...
/// the sandbox is already free for the next exec.
///
...
/// other backends.
///
/// Robustness contract: stdout/stderr are drained to EOF and control is read
/// until `Exit` **even if the IPC client disconnects mid-stream** (output frames
/// are then dropped and `ipc_alive` becomes `false`), so the guest protocol
/// always reaches a clean, reusable boundary. The exit payload is returned only
/// after stdout EOF + stderr EOF + `Exit`, so output is never truncated.
    ...
    W: AsyncWrite + Unpin,
{
    ...
    // them concurrently. Note: stdin is NOT shutdown up-front.
    ...
    // implies the sandbox is already free for the next exec.
...
/// implies the sandbox is already free for the next exec. Returns
...
///
/// Each stdout/stderr read produces a frame. Encoding into a fresh
/// `5 + payload.len()` buffer per call would mean an allocation plus a header
/// and full-payload memcpy per chunk — costly on large-output workloads (8 KiB
/// reads). Writing the header and payload as two back-to-back `write_all`s
/// avoids both; the kernel coalesces the two writes on localhost
...
mod tests {
    ...
    // ===== bridge stream/reconnect integration tests =========================
    //
    ...
    async fn stream_exec_forwards_frame_stdin_to_guest() {
        // Verify stdin forwarding: host sends FRAME_STDIN frames
        ...
        // IPC duplex: the test writes FRAME_STDIN frames; stream_exec_on_guest reads them.
