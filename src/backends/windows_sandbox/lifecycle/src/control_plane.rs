...
//! `pub use os::*;`, so paths such as `control_plane::process_creation_time`,
//! `control_plane::TransitionLock`, and `control_plane::HOST_VM_MUTEX_NAME`
//! resolve at the crate's `control_plane` module. The split is purely
//! organisational.
...
/// Ensure the state-aware record root exists, is owned by us, and carries an
/// owner-only DACL **before any trusted record is read out of it**. Fails closed
/// (refusing to proceed) if the root was pre-created by another user on a shared
/// temp directory — defeating a cross-user attacker who pre-plants a forged
/// `daemon.json` / `record.json` to misdirect teardown or leak the auth nonce.
/// Idempotent and cheap; safe to call at the start of every phase and at daemon
/// startup. See [`os::ensure_secure_dir`].
