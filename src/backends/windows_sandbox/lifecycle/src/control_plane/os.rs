...
//!
//! These primitives live in their own module so the control plane's
//! decision-logic layer (`plan_kill_set`, `classify_startup`,
//! `classify_stale_daemon_cleanup`, record schemas and CRUD) stays
//! navigable rather than buried under several hundred lines of `unsafe`
//! Win32 plumbing. Every public item is re-exported from [`super`] via
//! `pub use os::*;`, so call sites name them as
//! `control_plane::process_creation_time`, etc.
//!
...
///
/// **Same-user scope:** the mutex name is fixed and the
...
/// design.
///
/// **Why `Local\` and not `Global\`:** creating an object in the `Global\`
/// kernel namespace requires `SeCreateGlobalPrivilege`, which standard
/// (non-elevated) users do not hold — and `wxc-exec` / the daemon run
/// **unelevated** (host-prep owns the only elevated step). A `Global\` mutex
/// would therefore fail to create for the actual runtime identity. The slot is
/// only ever contended by processes in **one** interactive logon session: the
/// daemon is spawned by an `wxc-exec` start phase in the caller's session
/// (never session 0 / as a service), and one-shot runs in that same session, so
/// `Local\` is exactly the right blast radius. A cross-session collision (e.g. a
/// service-hosted daemon racing an interactive one-shot) is not a supported
/// deployment; if it ever becomes one, the daemon would need to run elevated and
/// switch to a `Global\` mutex with an explicit SD.
///
...
/// Stamp `dir` with an owner-only, inheritable DACL **and verify we own it**,
/// so any files / subdirs created underneath are not cross-user readable /
/// tamperable on a shared temp dir, AND a directory pre-created by another user
/// is *refused* rather than merely re-stamped.
///
/// Verifying ownership is essential: a foreign owner of a pre-existing directory
/// retains *implicit* `WRITE_DAC`, so stamping an owner-only DACL does not evict
/// them — they could re-grant themselves access and read the per-launch auth
/// nonce or forge an ownership record/marker we would then trust. Failing closed
/// when the owner is not us closes that cross-user hole. See
/// [`wxc_common::filesystem_dacl::owner_is_self`].
#[cfg(windows)]
pub fn set_owner_only_dir(dir: &Path) -> Result<()> {
    ...
    // After stamping the DACL, confirm WE own the directory. A foreign owner
    // (another user who pre-created this shared-temp dir) can defeat the DACL
    // via implicit WRITE_DAC, so its contents must not be trusted.
...
/// No-op stub so the crate still builds on non-Windows (`cargo check
/// --workspace`); the DACL primitives are Windows-only.
#[cfg(not(windows))]
pub fn set_owner_only_dir(_dir: &Path) -> Result<()> {
    Ok(())
}

/// Create `dir` (and parents) if needed, then secure + ownership-verify it via
/// [`set_owner_only_dir`]. Use before reading any *trusted* state (ownership
/// records / markers / per-launch nonce) out of a shared-temp root, so a
/// directory an attacker pre-created is refused before its contents are read.
#[cfg(windows)]
...
/// Non-Windows stub: create the directory; the DACL primitives are Windows-only.
#[cfg(not(windows))]
pub fn ensure_secure_dir(dir: &Path) -> Result<()> {
    std::fs::create_dir_all(dir).with_context(|| format!("create dir {dir:?}"))
}
