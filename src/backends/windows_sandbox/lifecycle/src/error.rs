...
pub(crate) enum OneShotError {
    ...
    /// channels to the guest agent. A single variant because the shared
    /// [`crate::vm::launch_managed_vm`] helper owns the whole boot sequence;
    /// per-phase variants (`Rendezvous`, `Connect`) would never be observed
