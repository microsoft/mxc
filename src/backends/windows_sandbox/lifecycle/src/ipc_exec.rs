...
/// The 5-byte data-frame header (`[kind][len: u32 LE]`) **without** the payload.
///
/// Lets a caller write the header and payload as two back-to-back `write_all`s
/// (the kernel/TCP coalesces them on localhost), avoiding a fresh per-frame
/// `Vec` allocation + full payload copy on the hot stdin-forwarding path. See
/// [`encode_frame`] for the all-in-one form.
pub fn frame_header(kind: u8, payload_len: usize) -> [u8; 5] {
    let mut h = [0u8; 5];
    h[0] = kind;
...
/// Read the framed [`ExecStart`] request (4-byte LE length prefix + JSON) from
/// an async stream — the request that follows the `EXEC <nonce>\n` line on the
/// daemon's IPC connection. The async-reader sibling of [`write_exec_start`] /
/// [`encode_exec_start`], kept here so the request framing lives next to its
/// encoder rather than being re-implemented inline in the daemon's control
/// server.
