...
//! Translate request policy into Windows Sandbox mapped folders and rejects.
//!
//! Windows Sandbox shares nothing by default. Mapped read/write and read-only
//! directories are additive; denied paths that overlap a mapped share are
//! rejected because Windows Sandbox has no per-path deny primitive. Network
//! isolation is currently all-or-nothing and enforced by the guest firewall.
...
pub(crate) struct WsbPolicyPlan {
    /// Extra host folders to expose inside the guest.
    pub(crate) mapped_folders: Vec<MappedFolder>,
}

/// Validate the request's policy and produce a [`WsbPolicyPlan`].
