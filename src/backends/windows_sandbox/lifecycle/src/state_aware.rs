...
/// [`SandboxState::Started`]. The cleanup:
///
...
///      to serialise against a concurrent one-shot launch/reconcile path, then
///      [`crate::vm::teardown_via_plan`] seeded with the stale proof. Daemon record
///      removed only on [`TeardownOutcome::ConfirmedGone`]; `StillRunning` /
///      `ProbeFailed` preserves the record and refuses.
...
///      sandbox A must never act on sandbox B's records.
fn cleanup_stale_daemon_orphan(sandbox_id: &str) -> Result<(), MxcError> {
    ...
    // Minimal current-thread runtime for the rare, short OS-touching cleanup calls.
    ...
    match plan {
        ...
        control_plane::StaleDaemonCleanup::Reclaim { proof } => {
            // Acquire HostVmLock for the snapshot-to-kill window:
...
/// Reap any orphan left by an interrupted `start` for a sandbox whose **own**
/// record is *not* `Started` (i.e. `Provisioned` / `Stopped`), gating every
/// action on daemon-record identity so a single crashed/unrelated sandbox can
/// neither strand a live VM nor fatally wedge stop/deprovision for everyone
/// else:
///
/// - **Live daemon for THIS sandbox** — `start` brought the daemon (and its VM)
///   up but the phase process died before persisting `Started`, leaving our
///   record at `Provisioned`. The VM is real and ours; send `IPC_STOP`, wait
///   for the daemon to exit, and remove its record.
/// - **Live daemon for a DIFFERENT sandbox** — another sandbox is currently
///   active and is none of our business. Leave it alone.
/// - **No live daemon, a stale record for THIS sandbox** — an interrupted-start
///   crash may have left an orphan VM that outlived its daemon; run the
///   positive-proof-gated [`cleanup_stale_daemon_orphan`].
/// - **No live daemon, a stale record for a DIFFERENT sandbox** — unrelated
///   bookkeeping; ignore it (do NOT call `cleanup_stale_daemon_orphan`, which
///   would classify the id mismatch as a fatal `backend_error` and wedge
///   stop/deprovision of every other provisioned/stopped sandbox).
/// - **No record at all** — nothing to reap.
fn reap_non_started_orphan(sandbox_id: &str) -> Result<(), MxcError> {
    match live_daemon().map_err(|e| MxcError::backend_error(format!("{e}")))? {
        ...
        Some(_) => {
            // A live daemon for a different sandbox — not ours; leave it.
            Ok(())
        }
        None => {
            let stale = read_daemon_record()
            ...
            match stale {
                ...
                // No record, or a stale record for an unrelated sandbox: ignore.
                _ => Ok(()),
            }
        }
    }
...
///
/// Length is locked to exactly 8 because that is what
/// [`wxc_common::id::mint_random_token`] produces and what the SDK + docs
/// promise (`wsb:<8-hex>`). A permissive 1-128 range would accept truncated
/// IDs like `wsb:1` that fall through to `not_provisioned` / `not_started`
/// instead of being rejected as `malformed_id`, obscuring programmer error --
/// a typo like `wsb:dead` becoming "not provisioned" is much less actionable
/// than "malformed id". If a future token format change (e.g. UUID hex) is
...
/// [`is_valid_sandbox_token`] for the grammar rationale; matches
...
/// (`^[a-f0-9]{8}$`). Extracted as a pure helper so the token grammar is
/// unit-testable.
...
/// Bounded flush policy for the non-TTY exec output path: flush after enough
/// bytes have accumulated OR enough wall-clock time has elapsed since the last
/// flush, so newline-free / carriage-return progress output reaches a pipe
/// consumer promptly without paying a `flush` syscall on every frame. TTY output
/// keeps its eager per-frame flush and does not use this gate.
struct FlushGate {
    last: Instant,
    pending: usize,
}
...
fn run_exec_stream(daemon: &DaemonRecord, request: &ExecutionRequest) -> Result<i32, MxcError> {
    ...
    // receive whatever the SDK / shell piped in. Stdin forwarding shares
    // the IPC stream with a detached writer thread that copies our stdin
    // into FRAME_STDIN frames.
    //
    // Two modes:
    // - TTY-stdin: drop with a loud stderr warning. The
    ...
    //   then build a ConPTY pump on the guest side.
    ...
    //   `execInSandbox` PTY users stop seeing typed input vanish without
    //   any signal. Reading the TTY here is not an option -- it would block
    //   until the user sends Ctrl-Z/Ctrl-D, which is not the exec contract.
    ...
    //
    // TODO(pty-plumbing): widen ExecStart with `tty: bool` and
            ...
            std::thread::spawn(move || {
                ...
                loop {
                    match stdin.read(&mut buf) {
                        Ok(0) => break,
                        Ok(n) => {
                            // Split-write the 5-byte header then the payload to
                            // avoid a fresh per-frame Vec allocation + copy on
                            // the stdin hot path (the two writes coalesce on
                            // localhost / TCP).
                            let header = ipc_exec::frame_header(ipc_exec::FRAME_STDIN, n);
                            if writer
                                .write_all(&header)
    ...
    // Only force-flush per frame when stdout/stderr is a TTY
    ...
    // Non-TTY streams use a bounded flush policy (see FlushGate) so streamed
    // progress is not stranded in the buffer until FRAME_EXIT.
...
impl StatefulSandboxBackend for WindowsSandboxRunner {
    ...
    fn provision(
        ...
        // Secure the record root (owner-verify + owner-only DACL) before
        // creating any per-sandbox state under it, so a shared-temp root an
        // attacker pre-created is refused (fails closed).
        ...
        // Lock the per-sandbox scratch dir down to owner-only (inheritable) AND
        // verify we own it, so the record.json (auth nonce) and any other state
        // written inside are not cross-user readable/tamperable when the temp
        // dir is shared.
