...
pub fn generate_wsb(
    ...
    // in the host-side rendezvous folder past stop / deprovision, and older
    // guest versions also logged each exec's raw `script_code` -- so an
    // attacker (or a curious admin)
...
:: Truncate any prior bootstrap.log content on this boot.
