...
pub fn generate_wsb(
    ...
    //
    // Truncate the log on every guest startup. The bootstrap log persists
    // in the host-side rendezvous folder past stop / deprovision; with
    // same-user read access to %TEMP%\wxc-wsb-stateaware-rendezvous an
    // attacker (or a curious admin) could otherwise read whatever it
    // contains. The guest does not log raw `script_code` (see executor.rs),
    // and truncating on each fresh boot keeps any prior contents from
...
"#,
  ...
  <Networking>Enable</Networking>
        ...
        host_python = xml_escape(&python_dir.display().to_string()),
...
/// Write `content` to `path`, removing any pre-existing file first.
///
/// The generated `bootstrap.cmd` / `.wsb` live in fixed-name files inside the
/// (owner-only DACL'd) rendezvous/config dirs. `std::fs::write` truncates an
/// existing file in place but preserves its explicit ACEs, so a cross-user
/// attacker who pre-created the file with a writable ACE could tamper with the
/// bootstrap command (which the guest executes) or the mapped-folder config.
/// Removing and recreating yields a fresh file inheriting only the parent's
/// owner-only DACL. Fails closed if a pre-existing file cannot be removed.
...
/// computed upstream by [`crate::control_plane::plan_kill_set`]. This is the
/// OS-touching half: issue the terminations and poll for confirmation.
/// Splitting the pure plan from the effectful kill keeps the one-shot and
/// daemon paths symmetric: neither enumerate-and-kills on an empty seed, so a
/// foreign VM that merely appears in the snapshot-to-kill window is never killed.
///
/// Returns a [`TeardownOutcome`] tri-state (see the enum for each variant) so
/// the caller knows whether it may delete the durable record: only on
/// [`TeardownOutcome::ConfirmedGone`]. `StillRunning` / `ProbeFailed` MUST
/// preserve the record so the next daemon's
/// [`crate::control_plane::classify_startup`] can reclaim by proof intersection.
///
/// Best-effort — termination errors are logged, not propagated; the outcome
/// reflects only what the polling loop can confirm. Liveness tracks the
/// `WindowsSandbox*` host processes; the SYSTEM-owned `vmmem*` residue is NOT awaited.
...
/// Caller-provided bookkeeping hooks for [`launch_managed_vm`]. The one-shot
/// runner (`one_shot::drive`) and the state-aware daemon
/// (`daemon::launch_and_connect`) share an identical nonce-write -> launch ->
/// capture-proof -> rendezvous -> connect sequence; this trait factors out the
/// per-caller ownership / proof bookkeeping seam so that sequence lives in one
/// place.
///
...
///
/// Used by both `one_shot::drive` (per-run marker file) and
/// `daemon::launch_and_connect` (daemon record); the differing per-caller
/// bookkeeping is supplied via the [`LaunchObserver`]. Pre-launch setup
/// (rendezvous-dir, .wsb generation, config-dir DACL) stays per-caller and is
/// assumed done: `wsb_path` must exist and `rendezvous_dir` must already carry
/// its owner-only DACL.
pub async fn launch_managed_vm(
    ...
    // threat model and the same-user-trusted scope.
    ...
    // `Owned(Vec::new())` -- empty Owned proof would defeat
    // intersection-only teardown semantics and leak
    // the VM at exit.
