...
/// `version` is the schema version stamped on the policy; it no longer selects
/// the backend — BaseContainer vs AppContainer is chosen by host capability.
...
// #1 (the process handle was closed before the wait completed) and #2 (a finite
// timeout killed only the direct child, so it never fired). ProcessContainer
// resolves to BaseContainer or AppContainer by host capability; these guards
// hold for whichever backend the host selects.
...
fn process_container_captures_stdout() {
    // Regression guard for #1: a valid exit code and captured stdout prove the
