...
///
/// Lifecycle operation and sandbox identity are supplied through the API or
/// executor command line, so the document contains one operation-neutral root.
pub fn development_schema() -> Value {
    let mut generator = SchemaGenerator::default();

    let dispatch = one_shot_dispatch(subschema::<OneShotRequest>(&mut generator));
    let mut definitions =
    ...
    add_property_alias(&mut definitions, "Request", "seatbelt", "macos_sandbox");
    exclude_duplicate_alias(&mut definitions, "Request", "seatbelt", "macos_sandbox");
...
mod tests {
    ...
    #[test]
    fn contains_the_operation_neutral_root() {
    ...
    #[test]
    fn dispatch_uses_containment_but_not_phase() {
        let schema = development_schema();
    ...
    #[test]
    fn root_pins_version_and_omits_lifecycle_dispatch_fields() {
        let schema = development_schema();
        let definitions = definitions(&schema);
        ...
        let request = &definitions["Request"];
        assert!(request["properties"].get("phase").is_none());
        assert!(request["properties"].get("sandboxId").is_none());
