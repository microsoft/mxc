...
pub struct ProcessContainerNetwork {
    ...
    /// the configured runtime proxy. The shared semantic parser validates that
    /// it is supported by the selected backend and accompanied by
...
pub struct ProcessContainer {
    ...
    /// Whether deny-and-record learning mode is enabled.
    #[serde(default)]
...
pub struct RuntimeConfig {
    /// Optional loopback proxy the runtime configures for the sandbox. The
    /// shared semantic parser validates its URL, loopback address, port, and
    /// required egress posture after adaptation.
    #[serde(default)]
