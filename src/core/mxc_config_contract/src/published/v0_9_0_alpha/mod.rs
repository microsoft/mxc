...
//! Wire types for the published `0.9.0-alpha` configuration contract.
//!
//! These types validate the JSON structure and value constraints of the
//! in-published contract. They preserve omitted optional fields for a later
...
string_enum! {
    ...
    pub enum Version {
        /// The published `0.9.0-alpha` contract.
        V0_9_0Alpha => ["0.9.0-alpha"],
    }
}

mod network;
/// The published `0.9.0-alpha` one-shot configuration contract.
mod one_shot;
mod primitives;
mod request;
#[cfg(feature = "schema-gen")]
mod schema;
mod stable;
/// The published `0.9.0-alpha` state-aware configuration contract.
mod state_aware;
...
#[cfg(feature = "schema-gen")]
pub use schema::published_schema;
pub use stable::{
