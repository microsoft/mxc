...
/// A validated request for the published `0.9.0-alpha` published contract.
#[derive(Debug)]
pub enum Request {
...
/// An error encountered while selecting or deserializing a published request.
...
/// containment requires a network posture that the request did not provide.
...
/// Selects and deserializes one exact published request from raw JSON source.
///
