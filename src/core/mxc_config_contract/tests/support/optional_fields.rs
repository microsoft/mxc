...
fn accepts_empty_optional_objects() {
    ...
    .into_iter()
    .chain(crate::OPTIONAL_EMPTY_OBJECT_ADDITIONS.iter().copied())
    {
        let json = format!(
            r#"{{
                "version": "0.9.0-alpha",
...
fn rejects_null_optional_fields() {
    ...
    let version_cases = crate::OPTIONAL_NULL_FIELD_ADDITIONS
        .iter()
        .map(|(name, invalid_fields)| (*name, version_and_process.as_str(), *invalid_fields));

    assert_invalid_cases(
        common_cases.into_iter().chain(version_cases),
        "null optional field",
    );
}
