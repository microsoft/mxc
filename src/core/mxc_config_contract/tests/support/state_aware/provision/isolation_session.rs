...
#[test]
fn rejects_invalid_network_action_types() {
    for value in ["null", "123", "true", "[]", "{}"] {
        for network_fields in [
            format!(
                r#""egress": {{"default": {value}}}, "ingress": {{"default": "allow", "hostLoopback": "allow"}}"#
            ),
            format!(
                r#""egress": {{"default": "allow"}}, "ingress": {{"default": {value}, "hostLoopback": "allow"}}"#
            ),
            format!(
                r#""egress": {{"default": "allow"}}, "ingress": {{"default": "allow", "hostLoopback": {value}}}"#
            ),
        ] {
...
fn rejects_unknown_network_fields() {
    for field in [
        ...
        let network_fields = format!(
            r#""egress": {{"default": "allow"}}, "ingress": {{"default": "allow", "hostLoopback": "allow"}}, {field}"#
        );
        assert_invalid(&request_with_network_fields(&network_fields));
    }
}
