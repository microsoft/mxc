// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.

#[path = "v0_10_0_alpha/annotations.rs"]
mod annotations;
#[path = "v0_10_0_alpha/common.rs"]
mod common;
#[path = "v0_10_0_alpha/enums.rs"]
mod enums;
#[path = "v0_10_0_alpha/experimental.rs"]
mod experimental;
#[path = "v0_10_0_alpha/fixtures.rs"]
mod fixtures;
#[path = "v0_10_0_alpha/network.rs"]
mod network;
#[path = "v0_10_0_alpha/one_shot.rs"]
mod one_shot;
#[path = "v0_10_0_alpha/optional_fields.rs"]
mod optional_fields;
#[path = "v0_10_0_alpha/request.rs"]
mod request;
#[path = "v0_10_0_alpha/seatbelt.rs"]
mod seatbelt;
#[path = "v0_10_0_alpha/state_aware.rs"]
mod state_aware;
