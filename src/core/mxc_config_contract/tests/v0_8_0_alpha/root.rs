    ...
    assert_invalid_cases(
        [
            ...
            (
                "runtimeConfig",
                version_and_process.as_str(),
                r#""runtimeConfig": {"network_proxy": "localproxy"}, "runtimeConfig": {"network_proxy": "remoteproxy"}"#,
            ),
            (
                "lxc",
                version_and_process.as_str(),
    ...
    assert_invalid_cases(
        [
            ...
            (
                "network.egress",
                version_and_process.as_str(),
                r#""network": {"egress": {"unknownField": true}}"#,
            ),
            (
                "network.egress.allow[]",
                version_and_process.as_str(),
                r#""network": {"egress": {"allow": ["{\"unknownField\": true}"]}}"#,
            ),
            (
                "network.egress.allow[].to[]",
                version_and_process.as_str(),
                r#""network": {"egress": {"allow": ["{\"to\": [{\"unknownField\": true}]}"]}}"#,
            ),
            (
                "network.egress.allow[].ports[]",
                version_and_process.as_str(),
                r#""network": {"egress": {"allow": ["{\"ports\": [{\"unknownField\": true}]"]}}"#,
            ),
            (
                "network.ingress",
                version_and_process.as_str(),
                r#""network": {"ingress": {"unknownField": true}}"#,
            ),
            (
                "ui",
                version_and_process.as_str(),
            ...
            (
                ...
                version_and_process.as_str(),
                r#""processContainer": {"network": {"unknownField": true}}"#,
            ),
            (
                "processContainer.ui",
                version_and_process.as_str(),
...
#[test]
fn rejects_adjacent_and_non_exact_versions() {
    for version in ["0.7.0-alpha", "0.8.0", "0.8.1-alpha", "0.9.0-alpha"] {
        let json = format!(
            r#"{{
                "version": "{version}",
                "process": {{"commandLine": "echo"}}
...
#[test]
fn rejects_invalid_process_container_capability_names() {
    for capability in [
