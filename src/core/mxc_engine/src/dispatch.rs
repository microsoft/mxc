...
//! (native PSEC or transitional SBOX, shared with the run-to-completion path
//! via `appcontainer_common::dispatcher`), Bubblewrap
...
fn spawn_process_container(
    ...
    // Streaming uses the same native PSEC/SBOX selection as run-to-completion.
    let capture_factory = crate::guarded_capture::factory_for_request(request);
    match dispatcher::spawn(request, logger, StdioMode::Pipes, capture_factory) {
...
fn spawn_process_container(
    ...
    Err(MxcError::unsupported_containment(
        "ProcessContainer (PSEC / SBOX) is only available on Windows",
    ))
}
