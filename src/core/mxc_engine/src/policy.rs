...
mod tests {
    ...
    fn wire_config_omits_ui_when_the_caller_supplied_none() {
        ...
        let config =
            super::build_wire_config(&policy, &super::Containment::Process, "echo hello", None)
    ...
    fn macos_allowed_hosts_without_outbound_is_accepted() {
        ...
        assert!(
            build_request(&policy, "echo hello", None).is_ok(),
    ...
    fn build_request_maps_filesystem_and_timeout() {
        ...
        let request =
            build_request(&policy, "echo hello", None).expect("build_request should succeed");
        ...
        assert!(request.inner.script_code.contains("echo hello"));
    }
    ...
    fn seatbelt_extra_mach_lookups_and_keychain_round_trip() {
        ...
        let mut request = build_request(&policy, "echo hello", None).expect("build_request");
    ...
    fn capture_denials_reaches_the_container_policy() {
        ...
        let request = build_request(&policy, "echo hello", None).expect("build_request");
    ...
    fn default_containment_resolves_the_host_backend() {
        ...
        let request = build_request(&minimal_policy(), "echo hello", None).expect("build_request");
