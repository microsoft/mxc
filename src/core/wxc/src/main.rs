...
use clap::Parser;
use processcontainer_common::appcontainer_runner::delete_app_container_profile;
...
fn main() {
    ...
    if cli.probe {
        ...
        let output = processcontainer_common::probe::run_probe(
            &request,
            mxc_engine::guarded_capture_available(),
        );
        // processcontainer_common has no dependency on the isolation-session
        ...
        match processcontainer_common::probe::to_json_pretty(&output) {
            Ok(s) => println!("{s}"),
            Err(e) => {
