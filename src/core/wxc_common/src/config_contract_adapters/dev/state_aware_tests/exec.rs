...
fn exec_preserves_process_network_telemetry_and_empty_values() {
    for id in [
        ...
        for extra in [
            ...
            let source = format!(
                r#"{{"version":"1.1.0-alpha","phase":"exec","sandboxId":"{id}","process":{{"commandLine":"echo hello","cwd":"/work","env":["FIRST=one","SECOND=two"],"timeout":60}}{extra}}}"#
            );
...
fn minimal_exec_keeps_omitted_process_fields_absent() {
    let (common, operation) = adapt(
        r#"{"version":"1.1.0-alpha","phase":"exec","sandboxId":"id","process":{"commandLine":"echo"}}"#,
    );
