...
pub(crate) enum AdaptedConfigRequest {
    OneShot(crate::config_input::ConfigInput),
...
pub(crate) fn adapt_request(request: contract::Request) -> Result<AdaptedConfigRequest, WxcError> {
    let input = match request {
        contract::Request::OneShot(request) => {
            return Ok(AdaptedConfigRequest::OneShot(one_shot::into_config_input(
                *request,
            )));
        }
...
pub(crate) fn one_shot_into_config_input(
    request: contract::OneShotRequest,
) -> crate::config_input::ConfigInput {
    one_shot::into_config_input(request)
}
