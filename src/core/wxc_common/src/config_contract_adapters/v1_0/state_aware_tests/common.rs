...
fn controlled_input_rejects_every_routing_and_one_shot_field() {
    for field in [
        ...
        let (mut common, _) =
            adapt(r#"{"version":"1.0.0","phase":"start","sandboxId":"iso:example"}"#);
        match field {
...
pub(super) fn assert_no_config_phase(phase: &str) {
    for id in [
        ...
        for fields in [
            ...
            let source =
                format!(r#"{{"version":"1.0.0","phase":"{phase}","sandboxId":"{id}"{fields}}}"#);
            ...
            assert_eq!(
                ...
                mxc_config_contract::ContractVersion::V1_0_0
            );
