...
pub fn load_mxc_request_with_options(
    ...
    let result = (|| {
        ...
        let json_str = if opts.cli_command.is_empty() {
            json_str
        } else {
            apply_cli_command(&json_str, opts.cli_command, logger)?
        };

        parse_mxc_request_json(&json_str, logger)
    })();
...
fn apply_cli_command(
    ...
    // The probed phase selects the failure convention for everything below:
    // a state-aware request emits a JSON envelope on stdout, a one-shot request
    // the stderr diagnostic. Deciding this before the command is rendered is
    // what keeps an unconvertible or empty command from silently switching a
    // state-aware failure onto the one-shot output path.
    let route: fn(String) -> ParseError = if phase.is_some() {
        |message| ParseError::StateAware(MxcError::malformed_request(message))
    } else {
        |message| ParseError::Decode(WxcError::ConfigParse(message))
    };
    ...
    let command = cmdline_from_argv_for_context(argv, context)
        .map_err(|e| route(format!("invalid CLI command override: {e}")))?;

    if command.is_empty() {
        return Err(route("CLI command override must not be empty".to_string()));
    }
...
mod tests {
    ...
    fn cli_command_preserves_duplicate_field_errors() {
        // The splice must not launder a duplicate member. A `serde_json::Value`
        // round-trip would silently keep the last one and reserialize a single
        // key, so `config_deserialize`'s duplicate rejection would never fire
    ...
    fn cli_command_preserves_invalid_command_line_type_errors() {
        // Overwriting a wrongly typed `commandLine` would make a document the
        // parser rejects today parse cleanly under an override.
    ...
    fn cli_command_preserves_the_source_line_of_later_errors() {
        // The localized splice keeps every byte offset outside the replaced
        // span, so serde's line/column diagnostics still describe the caller's
        // own document.
        let json = r#"{
  "process": {"commandLine": "policy.exe"},
  "network": {
    ...
    #[test]
    fn entry_point_command_failures_are_routed_by_the_probed_phase() {
        // Routing is behavior, not cosmetics: a state-aware request must keep
        // the JSON-envelope-on-stdout convention even when the failure is the
        // entry point's rather than the parser's. Deciding the convention from
        // the probed phase — before the command is rendered — is what prevents
        // an unconvertible or empty command from silently switching a
        // state-aware failure onto the one-shot stderr path.
        let one_shot = r#"{"process":{"commandLine":"policy.exe"}}"#;
        let state_aware = r#"{"phase":"exec","sandboxId":"iso:abcd1234"}"#;

        for (json, argv, expect_envelope) in [
            (one_shot, argv(&["app.exe", "hidden\0payload"]), false),
            (state_aware, argv(&["app.exe", "hidden\0payload"]), true),
            (one_shot, Vec::new(), false),
            (state_aware, Vec::new(), true),
        ] {
            let err = apply_cli_command(json, &argv, &mut Logger::new(Mode::Buffer)).unwrap_err();

            assert_eq!(
                matches!(err, ParseError::StateAware(_)),
                expect_envelope,
                "wrong output convention for {json}: {}",
                err.message()
            );
        }
    }
