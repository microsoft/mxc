...
fn parse_exact_development(json: &str, logger: &mut Logger) -> Result<MxcRequest, ParseError> {
    ...
    if let Err(error) = reject_removed_microvm(discriminator.containment) {
...
mod tests {
    ...
    fn differential_repository_corpus_has_no_unclassified_or_exact_looser_results() {
        ...
        // Phase One removes the sixteen legacy MicroVM fixtures from the corpus.
        let expected_inventory = (352, 328, 14);
        assert_eq!(
    ...
    fn state_aware_microvm_rejected_with_nvx_migration_guidance() {
        ...
        let error = match load_mxc(json) {
            Err(ParseError::StateAware(error)) => error,
            other => panic!("expected state-aware migration error, got {other:?}"),
        };
