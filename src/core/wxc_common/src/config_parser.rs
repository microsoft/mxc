...
pub fn load_one_shot_request_from_contract(
    ...
    let config = match request {
        ExactOneShotContract::V0_6(request) => {
            crate::config_contract_adapters::v0_6::into_config_input(*request)
        }
        ExactOneShotContract::V0_7(request) => {
            crate::config_contract_adapters::v0_7::into_config_input(*request)
        }
        ExactOneShotContract::V0_8(request) => {
            crate::config_contract_adapters::v0_8::into_config_input(*request)
        }
        ExactOneShotContract::V0_9(request) => {
            ...
            crate::config_contract_adapters::v0_9::one_shot_into_config_input(*request)
        }
        ExactOneShotContract::Dev(request) => {
            mxc_config_contract::dev::validate_one_shot_request(&request)
                .map_err(|error| WxcError::ConfigParse(error.to_string()))?;
            crate::config_contract_adapters::dev::one_shot_into_config_input(*request)
        }
    };

    let result = convert_config_input(config, logger, true, false);
...
fn parse_exact_published_one_shot<T>(
    json: &str,
    logger: &mut Logger,
    adapt: fn(T) -> crate::config_input::ConfigInput,
    ...
    T: serde::de::DeserializeOwned,
{
    ...
    convert_config_input(adapt(request), logger, true, false)
...
fn parse_exact_v0_9(json: &str, logger: &mut Logger) -> Result<MxcRequest, ParseError> {
    ...
    match adapted {
        crate::config_contract_adapters::v0_9::AdaptedConfigRequest::OneShot(config) => {
            convert_config_input(config, logger, true, false)
                .map(MxcRequest::OneShot)
                .map_err(ParseError::OneShot)
        }
...
fn parse_exact_mxc_request_json(json: &str, logger: &mut Logger) -> Result<MxcRequest, ParseError> {
    debug_assert_eq!(
        supported_versions().last().map(|version| version.as_str()),
    ...
    match probe_version(json).map_err(exact_version_error)? {
        ContractVersion::V0_6_0Alpha => parse_exact_published_one_shot(
            json,
            logger,
            crate::config_contract_adapters::v0_6::into_config_input,
        ),
        ContractVersion::V0_7_0Alpha => parse_exact_published_one_shot(
            json,
            logger,
            crate::config_contract_adapters::v0_7::into_config_input,
        ),
        ContractVersion::V0_8_0Alpha => parse_exact_published_one_shot(
            json,
            logger,
            crate::config_contract_adapters::v0_8::into_config_input,
        ),
...
/// Canonical latest exact schema version.
...
/// (`deny` > `enumerate` > `readonly` > `readwrite`):
///
...
fn present_backend_sections(cfg: &crate::config_input::ConfigInput) -> Vec<&'static str> {
...
fn convert_config_input(
    cfg: crate::config_input::ConfigInput,
    logger: &mut Logger,
...
fn normalize_state_aware_common(
    mut common: crate::config_input::ConfigInput,
    ...
    let mut request = convert_config_input(common, logger, require_process, state_aware_wslc_exec)?;
