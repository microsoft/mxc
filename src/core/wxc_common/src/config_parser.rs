...
use crate::models::{
    ...
    NetworkEnforcementMode, NetworkPolicy, PortMapping, Protocol, ProxyAddress, ProxyConfig,
    RuleAction, SeatbeltConfig, TelemetryConfig, TestFeatureConfig, UiPolicy, WindowsSandboxConfig,
    WslcConfig,
};
...
fn parse_proxy_url(field: &str, url_str: String) -> Result<ProxyConfig, WxcError> {
    ...
    // Only http/https are meaningful for the HTTP(S)_PROXY env vars we inject.
    // A non-HTTP scheme (ftp://, socks5://, ...) is silently ignored by many
    // clients, which fails open under WSLc's defaultPolicy=allow.
    let scheme = parsed.scheme();
    if scheme != "http" && scheme != "https" {
        // Redact any embedded userinfo (`user:password@`) before it reaches the
        // diagnostic/log stream — a proxy URL commonly carries basic-auth
        // credentials, and the scheme alone diagnoses the failure.
        let redacted = crate::proxy_env::redact_proxy_url(&url_str);
        return Err(WxcError::ConfigParse(format!(
            "{field} must use the 'http' or 'https' scheme (got '{scheme}'): {redacted}"
        )));
    }
...
mod tests {
    ...
    #[test]
    fn proxy_http_form_rejects_non_http_scheme() {
        // The `http` proxy form shares the same URL parser as `url`, so the
        // scheme guard must reject non-HTTP schemes there too — otherwise the
        // GA `network.proxy.http` surface would be a way around it.
        let json = r#"{
            "process": {"commandLine": "echo hi"},
            "containment": "processcontainer",
            "network": {"proxy": {"http": "socks5://proxy.example:1080"}}
        }"#;
        let encoded = base64_encode(json.as_bytes());
        let mut logger = test_logger();
        let err = load_request(&encoded, &mut logger, true).unwrap_err();
        ...
        assert!(
            msg.contains("network.proxy.http"),
