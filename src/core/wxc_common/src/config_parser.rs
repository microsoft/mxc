...
pub fn load_request(
    ...
    let result = (|| {
        let json_str = decode_request_input_without_logging(input, is_base64)?;
...
pub fn load_mxc_request_with_options(
    ...
    let result = (|| {
        ...
        if opts.cli_command.is_empty() {
            return parse_mxc_request_json(&json_str, logger);
        }

        match apply_cli_command(&json_str, opts.cli_command, logger) {
            Ok(effective_json) => {
                let source_map = effective_json.edit.as_ref().map(|edit| SourceMap {
                    original: &json_str,
                    edit,
                });
                parse_mxc_request_json_with_source_map(&effective_json.json, source_map, logger)
            }
            Err(cli_error) => parse_before_cli_error(&json_str, cli_error, logger),
        }
    })();
...
pub fn load_mxc_request_from_json(
    ...
    let result = parse_mxc_request_json(json_str, logger);
    if let Err(error) = &result {
        log_error(logger, &error.message(), error.output());
    }
    result
...
// The placeholder exists only to let an exec request reach typed validation.
// Do not emit the policy-command override log for this synthetic replacement.
const CLI_VALIDATION_COMMAND: &str = "__mxc_cli_validation_command__";
...
fn parse_before_cli_error(
    json: &str,
    cli_error: ParseError,
    logger: &mut Logger,
) -> Result<MxcRequest, ParseError> {
    if let Some(spliced) = crate::splice::splice_command(json, CLI_VALIDATION_COMMAND)
        .filter(|spliced| !spliced.replaced_existing)
    {
        let source_map = SourceMap {
            original: json,
            edit: &spliced.edit,
        };
        return match parse_mxc_request_json_with_source_map(&spliced.json, Some(source_map), logger)
        {
            Ok(_) => Err(cli_error),
    ...
    match parse_mxc_request_json(json, logger) {
        Ok(_) => Err(cli_error),
        Err(parse_error) => Err(parse_error),
    }
}
...
fn apply_cli_command(
    ...
    let Some(spliced) = crate::splice::splice_command(json, &command) else {
        let message = "CLI command override could not be applied to process.commandLine";
        return Err(match phase {
            None => ParseError::Decode(WxcError::ConfigParse(message.to_string())),
            Some(_) => ParseError::StateAware(MxcError::malformed_request(message)),
        });
    };
...
fn deserialize_with_source_map<'de, T>(
    json: &'de str,
    source_map: Option<SourceMap<'_>>,
) -> Result<T, config_deserialize::ConfigDeserializeError>
where
    T: Deserialize<'de>,
{
    config_deserialize::from_str(json).map_err(|error| match source_map {
        Some(source_map) => config_deserialize::remap_error_through_edit(
            error,
            json,
            source_map.original,
            source_map.edit,
...
fn parse_mxc_request_json(json_str: &str, logger: &mut Logger) -> Result<MxcRequest, ParseError> {
    parse_mxc_request_json_with_source_map(json_str, None, logger)
}
...
fn parse_mxc_request_json_with_source_map(
    json_str: &str,
    source_map: Option<SourceMap<'_>>,
    logger: &mut Logger,
) -> Result<MxcRequest, ParseError> {
    let discriminator: RequestDiscriminator<'_> = deserialize_with_source_map(json_str, source_map)
        .map_err(|error| ParseError::Decode(WxcError::ConfigParse(error.to_string())))?;

    if discriminator.phase.is_some() {
        let raw: serde_json::Value = deserialize_with_source_map(json_str, source_map)
            .map_err(|error| ParseError::Decode(WxcError::ConfigParse(error.to_string())))?;
        ...
        convert_wire_state_aware(json_str, discriminator.experimental, source_map, logger)
            .map(MxcRequest::StateAware)
            .map_err(|e| ParseError::StateAware(MxcError::malformed_request(e.to_string())))
    } else {
        let cfg: wire::MxcConfig = deserialize_with_source_map(json_str, source_map)
...
fn convert_wire_state_aware(
    ...
    let mut cfg: wire::MxcConfig = deserialize_with_source_map(&base_json, source_map)
        .map_err(|error| WxcError::ConfigParse(error.to_string()))?;
    ...
    Ok(ParsedStateAwareRequest {
        ...
        source_text: Some(
            source_map
                .map_or(json, |source_map| source_map.original)
                .to_owned()
                .into_boxed_str(),
        ),
    })
...
mod tests {
    ...
    #[test]
    fn cli_command_supplies_a_null_one_shot_process_block() {
        let json = r#"{"process": null}"#;

        assert!(
            load_mxc(json).is_err(),
            "process:null must remain invalid without a CLI command"
        );
    ...
    #[test]
    fn cli_command_supplies_a_null_state_aware_exec_process_block() {
        let json = r#"{
            "phase": "exec",
            "sandboxId": "iso:abcd1234",
            "process": null
        ...
        match load_mxc_with_cli(json, &argv(&["app.exe", "--flag"])).unwrap() {
            MxcRequest::StateAware(parsed) => {
                ...
                assert_eq!(parsed.request.script_code, "app.exe --flag");
            }
            MxcRequest::OneShot(_) => panic!("expected state-aware"),
        }
    }
    ...
    #[test]
    fn cli_command_preserves_the_source_line_of_later_errors() {
        let json = r#"{
            "process": {"commandLine": "policy.exe"},
            "network": {
                "defaultPolicy": 42
        ...
        let original_error = load_mxc(json).unwrap_err().message();
        assert!(
            original_error.contains("line 4 column"),
            "test input should fail on its fourth line: {original_error}"
        );

        let override_error = load_mxc_with_cli(json, &argv(&["cli.exe"]))
            .unwrap_err()
            .message();
        assert!(
            override_error.contains("line 4 column"),
            "CLI override moved the source location: {override_error}"
        );
    }

    #[test]
    fn cli_command_preserves_the_source_column_after_a_longer_replacement() {
        let json = r#"{"process":{"commandLine":"x"},"network":{"defaultPolicy":42}}"#;

        let without_cli = load_mxc(json).unwrap_err().message();
        let with_cli = load_mxc_with_cli(json, &argv(&["a-much-longer-command.exe", "--flag"]))
            .unwrap_err()
            .message();

        assert_eq!(with_cli, without_cli);
    }

    #[test]
    fn cli_command_preserves_the_source_column_after_a_shorter_replacement() {
        let json = r#"{"process":{"commandLine":"a-very-long-policy-command.exe --flag"},"network":{"defaultPolicy":42}}"#;

        let without_cli = load_mxc(json).unwrap_err().message();
        let with_cli = load_mxc_with_cli(json, &argv(&["x"]))
            .unwrap_err()
            .message();
    ...
    #[test]
    fn cli_command_preserves_non_exec_policy_error_precedence() {
        let json = r#"{
            "phase": "start",
            "sandboxId": "iso:abcd1234",
            "process": 42
        ...
        let with_cli = load_mxc_with_cli(json, &argv(&["app.exe", "--flag"])).unwrap_err();
        assert!(
            without_cli.message().contains("process"),
            "unexpected baseline error: {}",
            without_cli.message()
        );
        assert_eq!(with_cli.message(), without_cli.message());
        assert!(
            matches!(with_cli, ParseError::StateAware(_)),
            "expected a state-aware parse error"
        );
    }

    #[test]
    fn cli_command_preserves_exec_policy_error_precedence_when_sandbox_id_is_missing() {
        let json = r#"{
            "phase": "exec",
            "process": 42
        }"#;
    ...
    #[test]
    fn cli_command_rejects_valid_non_exec_request_after_validation() {
        let json = r#"{
        "phase": "start",
        "sandboxId": "iso:abcd1234"
    }"#;

        let error = load_mxc_with_cli(json, &argv(&["app.exe", "--flag"])).unwrap_err();

        assert!(matches!(error, ParseError::StateAware(_)));
        assert!(
            error
                .message()
                .contains("CLI command override is only supported for state-aware exec requests"),
            "unexpected error: {}",
            error.message()
        );
    }
    ...
    #[test]
    fn cli_command_reports_missing_sandbox_id_after_valid_exec_source() {
        let json = r#"{
        "phase": "exec",
        "process": {"commandLine": "policy.exe"}
    }"#;
        ...
        assert!(
            error
                .message()
                .contains("state-aware requests require 'sandboxId'"),
            "unexpected error: {}",
            error.message()
        );
    }
    ...
    #[test]
    fn cli_command_reports_missing_sandbox_id_when_it_would_supply_the_command() {
        let json = r#"{
        "phase": "exec",
        "process": {"cwd": "C:\\tmp"}
    }"#;
        ...
        assert!(!error.message().contains("commandLine"));
    }

    #[test]
    fn cli_command_preserves_existing_null_command_error_precedence_when_sandbox_id_is_missing() {
        let json = r#"{
            "phase": "exec",
            "process": {"commandLine": "bad\u0000command"}
        }"#;

        let without_cli = load_mxc(json).unwrap_err();
        let with_cli = load_mxc_with_cli(json, &argv(&["app.exe", "--flag"])).unwrap_err();
        assert!(
            without_cli.message().contains("commandLine"),
            "unexpected baseline error: {}",
            without_cli.message()
        );
        assert_eq!(with_cli.message(), without_cli.message());
    ...
    #[test]
    fn cli_command_preserves_existing_null_command_error_when_override_succeeds() {
        let json = r#"{
            "phase": "exec",
            "sandboxId": "iso:abcd1234",
            "process": {"commandLine": "bad\u0000command"}
    ...
    fn apply_cli_command_splices_into_a_one_shot_request() {
        ...
        let doc: serde_json::Value = serde_json::from_str(&out.json).unwrap();
    ...
    #[test]
    fn apply_cli_command_does_not_log_when_the_process_is_null() {
        let mut logger = Logger::new(Mode::Buffer);

        let output = apply_cli_command(
            r#"{"process":null}"#,
            &argv(&["app.exe", "--flag"]),
            &mut logger,
        )
        ...
        let document: serde_json::Value = serde_json::from_str(&output.json).unwrap();
        assert_eq!(document["process"]["commandLine"], "app.exe --flag");
        assert!(!logger
            .get_buffer()
            .contains("Overriding policy process.commandLine"));
    }
    ...
    fn apply_cli_command_returns_the_source_unchanged_when_it_cannot_classify() {
        // Each passthrough path has an unreadable discriminator. Assert the
    ...
    #[test]
    fn apply_cli_command_defers_an_unspliceable_process_representation() {
        let error = apply_cli_command(
        ...
        assert!(
            error
                .message()
                .contains("could not be applied to process.commandLine"),
            "unexpected error: {}",
            error.message()
        );
    }
    ...
    #[test]
    fn cli_command_rejects_a_positional_process_sequence_instead_of_ignoring_the_override() {
        let json = r#"{"process":["policy.exe",null,null,null]}"#;

        let without_cli =
            load_mxc(json).expect("sanity: serde currently accepts the positional representation");
        let MxcRequest::OneShot(request) = without_cli else {
            panic!("expected one-shot");
        };
        assert_eq!(request.script_code, "policy.exe");

        let error = load_mxc_with_cli(json, &argv(&["cli.exe", "--flag"])).unwrap_err();
        assert!(
            error
                .message()
                .contains("could not be applied to process.commandLine"),
    ...
    #[test]
    fn cli_command_routes_an_unsupported_positional_exec_sequence_to_an_envelope() {
        let json = r#"{
            "phase": "exec",
            "sandboxId": "iso:abcd1234",
            "process": ["policy.exe", null, null, null]
        }"#;
