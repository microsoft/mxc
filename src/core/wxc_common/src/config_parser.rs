...
use crate::models::{
    ...
    DevelopmentConfig, ExecutionRequest, LifecycleConfig, LxcConfig, NetworkEnforcementMode,
...
/// directly.
///
/// This loader enforces exact registered contracts.
...
fn convert_wire_config(
    ...
    // Development-only fields are parsed from their permanent locations but
    // applied only when the caller authorizes experimental execution.
    let development = {
        let test = cfg
            .test
            .map(|value| TestFeatureConfig::from_raw(value.message));
        let windows_sandbox = cfg.windows_sandbox.map(|sb| {
...
mod tests {
    ...
    fn exact_parser_does_not_fallback_to_later_contracts() {
        for (case, json, expected_message) in [
            ...
            (
                "v0.8 rejects the removed experimental block",
                r#"{
                    "version": "0.8.0-alpha",
                    "process": {"commandLine": "echo hello"},
                    "experimental": {}
    ...
    fn exact_loader_preserves_development_configuration_and_telemetry() {
        for telemetry_enabled in [false, true] {
            ...
            assert_development_configuration(&parsed, telemetry_enabled);
            assert!(parsed.request().development.test.is_none());
            assert!(parsed.request().development.windows_sandbox.is_none());
            assert!(parsed.request().development.wslc.is_none());
        }
    }
