...
//! Test-only observations and diagnostic reference for the rolling parser.
...
pub(crate) struct LegacyStateAwareWireInput {
    pub config: crate::wire::MxcConfig,
...
#[derive(Debug, Clone)]
pub(crate) struct LegacyStateAwareRequest {
...
#[derive(Debug, Clone)]
pub(crate) enum LegacyMxcRequest {
    OneShot(ExecutionRequest),
    StateAware(LegacyStateAwareRequest),
}

#[cfg(test)]
mod tests {
    use super::LegacyStateAwareRequest as ParsedStateAwareRequest;
    use super::*;
