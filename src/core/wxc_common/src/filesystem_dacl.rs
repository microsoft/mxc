...
fn current_token_owner_sid() -> Result<OwnedSid, DaclError> {
    ...
    let token_path = Path::new("<process token owner>");
    unsafe {
        let mut token = HANDLE::default();
        OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &mut token)
            .map_err(|e| win32_err_str(token_path, &format!("OpenProcessToken: {e}")))?;
        ...
        let _ = CloseHandle(token);
        info.map_err(|e| {
            win32_err_str(token_path, &format!("GetTokenInformation(TokenOwner): {e}"))
        })?;

        let to = &*(buf.as_ptr() as *const TOKEN_OWNER);
        let mut pstr = PWSTR::null();
        ConvertSidToStringSidW(to.Owner, &mut pstr)
            .map_err(|e| win32_err_str(token_path, &format!("ConvertSidToStringSidW: {e}")))?;
        let sid_string = pstr
            .to_string()
            .map_err(|e| DaclError::InvalidSid(format!("token owner SID string not UTF-16: {e}")));
        if !pstr.is_null() {
            let _ = LocalFree(Some(HLOCAL(pstr.0 as *mut c_void)));
        }
        OwnedSid::parse(&sid_string?)
