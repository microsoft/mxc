...
impl From<crate::wire::IsolationSessionUser> for IsolationSessionUser {
    fn from(u: crate::wire::IsolationSessionUser) -> Self {
        // Destructure (no `..`) so a new wire field fails to compile until mapped.
        let crate::wire::IsolationSessionUser { upn, wam_token } = u;
        Self { upn, wam_token }
    }
}
...
/// local sandboxes. The one-shot surface takes no backend configuration.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
