...
//!
//! See `docs/telemetry/telemetry-policy.md` for the admin-facing reference and
...
mod platform {
    ...
    /// `docs/telemetry/telemetry-policy.md`; changing it breaks deployed
    /// policy.
    ...
    fn debug_policy_key_override() -> Option<String> {
        std::env::var("MXC_TEST_POLICY_KEY_OVERRIDE")
            .ok()
            .filter(|s| !s.is_empty())
    }

    /// The production key path is never exercised by the rest of the suite —
    /// every test redirects to `HKCU` via `MXC_TEST_POLICY_KEY_OVERRIDE` — so
    /// this guards the one property of it that silently breaks administrators.
    #[cfg(test)]
    mod key_path {
...
// ---------------------------------------------------------------------------
// Test support — the policy location is process-global state on Windows, so
// tests that redirect it must serialize against each other.
// ---------------------------------------------------------------------------
...
pub mod test_support {
    ...
    /// Serializes access to the debug-only policy-key override, which is
    /// process-global. Held for the whole lifetime of [`PolicyKeyGuard`] so a
    /// caller cannot construct the guard without also taking the lock.
    ...
    impl PolicyKeyGuard {
        ...
        pub fn new() -> Self {
            ...
            let previous = std::env::var_os("MXC_TEST_POLICY_KEY_OVERRIDE");
            ...
            std::env::set_var("MXC_TEST_POLICY_KEY_OVERRIDE", &subkey);
    ...
    impl Drop for PolicyKeyGuard {
        fn drop(&mut self) {
            ...
            match &self.previous {
                Some(v) => std::env::set_var("MXC_TEST_POLICY_KEY_OVERRIDE", v),
                None => std::env::remove_var("MXC_TEST_POLICY_KEY_OVERRIDE"),
            }
        }
    }
}
