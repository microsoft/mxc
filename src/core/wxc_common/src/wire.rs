...
/// Normalized development feature settings constructed by exact adapters.
#[derive(Debug, Clone, Default, Serialize)]
...
///
/// The only phase that carries backend-specific configuration into common
/// normalization. Keeping it in a phase-specific type avoids implying that
/// start, exec, stop, or deprovision carry the same internal data. Exact
/// request types separately own the external schema.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
