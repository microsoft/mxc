...
//! Windows service framework wiring for `mxc-denial-shim`.
//!
...
fn service_main(_args: Vec<OsString>) {
    if let Err(e) = run_service() {
        eprintln!("[mxc-denial-shim] service exited with error: {e}");
    }
}
...
fn run_service() -> Result<(), Box<dyn Error>> {
    ...
    let status_handle = service_control_handler::register("MxcDenialShim", event_handler)?;
