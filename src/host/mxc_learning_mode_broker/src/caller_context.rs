...
//! The broker enforces two access invariants:
//!
...
//!    `PROCESS_QUERY_LIMITED_INFORMATION` on the target, the broker
...
/// construction, the broker has already called `RevertToSelf` — the
/// caller's identity lives in this struct, not on the thread token,
/// so downstream ETW work runs with the broker's `LocalService` +
...
/// with the broker's own (`LocalService`) identity.
...
/// dropped the impersonation token). The broker's `LocalService`
