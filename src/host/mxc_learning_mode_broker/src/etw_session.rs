...
//! Privileged ETW-session creation owned by the broker.
//!
...
/// A live ETW session held by the broker while the caller consumes it.
///
/// Phase 2.2: the broker creates and immediately disengages — the caller
...
impl LiveSession {
    ...
    pub fn stop(self) {
        ...
        if status != WIN32_ERROR(0) {
            eprintln!(
                "[mxc-learning-mode-broker] ControlTraceW(STOP) for {} failed: {:#X}",
                self.name, status.0
            );
        }
    }
...
/// `ControlTraceW(NULL, name, QUERY)` (the broker does not retain
