...
//! `mxc-learning-mode-shim` — privileged service that loans ETW session-creation
...
//! `learning_mode_windows::wire::PIPE_NAME`) with an ACL that admits only
...
//! ## Operating modes
//!
//! - `mxc-learning-mode-shim` (no flags) — service entry point. Invoked by SCM
//!   when the service starts. Begins the named-pipe accept loop.
//! - `mxc-learning-mode-shim --debug` — runs the service main interactively in
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if debug {
        eprintln!("[mxc-learning-mode-shim] running in --debug mode (no SCM)");
