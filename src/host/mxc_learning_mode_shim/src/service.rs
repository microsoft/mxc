...
//! Windows service framework wiring for `mxc-learning-mode-shim`.
//!
...
fn service_main(_args: Vec<OsString>) {
    if let Err(e) = run_service() {
        eprintln!("[mxc-learning-mode-shim] service exited with error: {e}");
    }
}
...
fn run_service() -> Result<(), Box<dyn Error>> {
    ...
    let status_handle = service_control_handler::register("MxcLearningModeShim", event_handler)?;
