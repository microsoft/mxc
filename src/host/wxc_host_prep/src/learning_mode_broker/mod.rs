...
//! Install / uninstall / inspect the `MxcLearningModeBroker` Windows service.
//!
//! The broker itself lives in `mxc-learning-mode-broker.exe` (built from the
//! `mxc_learning_mode_broker` crate). `wxc-host-prep` is the supported way to
//! register / deregister it with SCM, matching the existing
//! `prepare-system-drive` / `prepare-null-device` admin-time-only
//! pattern.
//!
//! Conventions:
//! - **Service name**: `MxcLearningModeBroker` (matches the constant the broker
//!   binary itself uses with `start_dispatcher`).
//! - **Display name**: `"MXC Denial Capture Broker"` (visible in
//!   services.msc).
//! - **Account**: `NT AUTHORITY\LocalService` — least-privilege. The
//!   account doesn't carry `SeSystemProfilePrivilege` by default, so
//!   `install-learning-mode-broker` grants it explicitly via the LSA
...
//!   caller or by an explicit `Start-Service MxcLearningModeBroker`).
//! - **Install location**: the broker binary is **copied into a protected
//!   `Program Files` subdirectory** (`%ProgramFiles%\Mxc\mxc-learning-mode-broker.exe`)
...
//!   service binary.
//! - **Source binary**: `mxc-learning-mode-broker.exe` next to
//!   `wxc-host-prep.exe` (the SDK bin dir). Override the *source* with
//!   `--broker-path`; install always copies that source into `Program Files`.
...
const SERVICE_DISPLAY_NAME: &str = "MXC Denial Capture Broker";
const SHIM_BINARY_FILENAME: &str = "mxc-learning-mode-broker.exe";

/// `Program Files` subdirectory the broker binary is installed into.
...
/// time so the broker can call `StartTraceW`.
const SERVICE_ACCOUNT: &str = "NT AUTHORITY\\LocalService";

/// Default *source* path: `<wxc-host-prep dir>\mxc-learning-mode-broker.exe`.
fn default_broker_binary_path() -> Result<PathBuf, String> {
...
/// explicit `--broker-path` overrides the default SDK-bin-dir location; the
/// source must exist on disk.
fn resolve_source_broker_path(override_path: Option<&str>) -> Result<PathBuf, String> {
    match override_path {
        Some(p) => {
            let pb = PathBuf::from(p);
            if !pb.exists() {
                return Err(format!("broker binary not found at {}", pb.display()));
            }
            Ok(pb)
        }
        None => {
            let pb = default_broker_binary_path()?;
            if !pb.exists() {
                return Err(format!(
                    "broker binary not found at default path {}\n\
                     pass --broker-path <path> to specify an explicit source location",
                    pb.display()
                ));
            }
            Ok(pb)
...
/// `%ProgramFiles%\Mxc` — the install directory for the broker binary.
fn broker_install_dir() -> Result<PathBuf, String> {
...
/// Copies the source broker binary to the install target, overwriting any
/// existing copy. The destination inherits the `Program Files` DACL
/// (administrators / SYSTEM / TrustedInstaller write only).
fn copy_broker_to_target(source: &Path, target: &Path) -> Result<(), String> {
...
fn verify_target_hardening(target: &Path, install_root: &Path) {
    if !target.starts_with(install_root) {
        eprintln!(
            "warning: broker target {} is not under {}; protected ACL inheritance is not guaranteed",
            target.display(),
    ...
    for (sid, label) in BROAD_PRINCIPALS {
        if let Ok(aces) = wxc_common::filesystem_dacl::scan_explicit_aces_for_sid(target, sid) {
            for ace in aces {
                ...
                {
                    eprintln!(
                        "warning: installed broker {} grants write access to {label} \
...
/// Implements `wxc-host-prep install-learning-mode-broker`.
///
/// Copies the source broker binary into `%ProgramFiles%\Mxc` and registers
...
pub fn run_install(broker_path_override: Option<&str>) -> i32 {
    let source_path = match resolve_source_broker_path(broker_path_override) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("error: {e}");
            return 1;
    ...
    let install_dir = match broker_install_dir() {
        Ok(p) => p,
        Err(e) => {
            eprintln!("error: {e}");
            return 3;
        ...
        match existing.query_config() {
            Ok(cfg) => {
                ...
                if paths_match(&existing_path, &new_path) {
                    ...
                    if let Err(e) = copy_broker_to_target(&source_path, &target_path) {
                        eprintln!("error: {e}");
                        return 1;
                    }
                ...
                eprintln!(
                    "error: service {SERVICE_NAME} already installed with a different binary:\n  \
                     ...
                     run `wxc-host-prep uninstall-learning-mode-broker` first"
                );
                return 1;
            }
            Err(e) => {
...
/// Implements `wxc-host-prep uninstall-learning-mode-broker`.
///
...
pub fn run_uninstall() -> i32 {
    ...
    match service.delete() {
        Ok(_) => {
            ...
            if let Ok(install_dir) = broker_install_dir() {
...
/// Implements `wxc-host-prep dump-learning-mode-broker`.
///
