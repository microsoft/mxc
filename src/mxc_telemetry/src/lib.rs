...
mod provider {
    ...
    // Provider definition generated by build.rs — includes `id(...)`,
    // optionally `group_id(...)`, and the paired `MXC_EVENT_KEYWORD` /
    // `MXC_PRIVACY_TAG` constants used by every `write_event!` call below.
    //
    // These events support **both** local-only ETW and Microsoft telemetry
    // (UTC/Asimov) routing from the same provider, selected by a single
    // build-time signal (`MXC_TELEMETRY_PROVIDER_GROUP_GUID`, see
    // `build.rs`/`provider_codegen.rs`):
    //
    // - Unset (default: public/OSS/local dev builds) — no `group_id(...)`;
    //   `MXC_EVENT_KEYWORD` is a provider-local bit with no UTC meaning, and
    //   `MXC_PRIVACY_TAG` is `0` (not telemetry-classified). Microsoft's own
    //   WSL OSS repo follows the same pattern, zeroing these telemetry
    //   constants for non-official builds.
    // - Set to the real Microsoft telemetry group GUID (a deliberate,
    //   internal-only opt-in) — `group_id(...)` is present and the
    //   constants become `MICROSOFT_KEYWORD_MEASURES` /
    //   `PDT_PRODUCT_AND_SERVICE_USAGE`, matching WIL's
    //   `traceloggingconfig.h` / `MicrosoftTelemetry.h` conventions.
    //
    // Keeping the `PartA_PrivTags` field present in both modes (value only
    // differs) gives a stable event schema regardless of build.
...
mod provider_codegen_tests {
    ...
    /// The event-metadata constants must stay in lockstep with the
    /// `group_id` presence — a build can never end up telemetry-routed but
    /// untagged, or tagged but not routed.
    #[test]
    fn absent_guid_yields_local_only_event_metadata() {
        let consts = generate_event_metadata_consts(None);
        assert!(consts.contains("MXC_EVENT_KEYWORD: u64 = 0x1"));
        assert!(consts.contains("MXC_PRIVACY_TAG: u64 = 0x0"));
    }

    #[test]
    fn empty_guid_yields_local_only_event_metadata() {
        let consts = generate_event_metadata_consts(Some(""));
        assert!(consts.contains("MXC_EVENT_KEYWORD: u64 = 0x1"));
        assert!(consts.contains("MXC_PRIVACY_TAG: u64 = 0x0"));
    }
    ...
    fn valid_guid_yields_telemetry_event_metadata() {
        let consts = generate_event_metadata_consts(Some("7f10def4-a258-5fea-510e-2c3bb976687f"));
        assert!(consts.contains("MXC_EVENT_KEYWORD: u64 = 0x0000_4000_0000_0000"));
        assert!(consts.contains("MXC_PRIVACY_TAG: u64 = 0x0000_0000_0200_0000"));
    }
}
