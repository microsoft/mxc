...
// `cached_has_daemon` / `HAS_DAEMON` / the `has_daemon` import were
// removed in review H13: the one-shot `test_windows_sandbox` suite no
// longer pre-spawns a daemon (the legacy positional CLI it used is
// gone). State-aware coverage lives in
...
fn test_windows_sandbox() {
    ...
    // No daemon prerequisite (review H13): the one-shot path launches a
    // fresh disposable VM per call directly; the legacy warm-reuse daemon
    // contract this suite used to depend on is gone. State-aware
    // coverage (provision -> start -> exec -> stop -> deprovision) lives
    // in tests/scripts/run_windows_sandbox_state_aware_tests.ps1.
...
fn windows_sandbox_suite() {
    // Review H13: previously this suite pre-started a daemon via
    // `SandboxDaemon::start()` using the deleted positional CLI
    // (`daemon "wxc-windows-sandbox" "300000"`) -- structurally broken
    // against the new daemon contract which requires `--token <id>`
    // plus a nonce on stdin and is owned by the state-aware lifecycle,
    // not by one-shot. Even with the spawn syntax fixed, the new
    // one-shot path returns `Busy` when a live state-aware daemon
    // owns the host VM slot. The daemon pre-spawn is therefore
    // removed entirely; each `run_wxc_config` below launches its own
    // fresh disposable VM via the one-shot path.
    let cases = [
        SandboxCase {
...
// `SandboxDaemon` was removed in review H13. The legacy positional CLI
// (`daemon "wxc-windows-sandbox" "300000"`) is no longer accepted; the
// new daemon requires `--token <id>` plus a nonce on stdin and is owned
// by the state-aware lifecycle. The one-shot suite above no longer
// needs a pre-spawned daemon -- each `run_wxc_config` launches its own
// fresh disposable VM. State-aware coverage lives in
// `tests/scripts/run_windows_sandbox_state_aware_tests.ps1`.
