...
impl AppContainerScriptRunner {
    ...
    ///
    /// Used by the dispatcher to avoid a second `ConvertSidToStringSidW`
    /// round-trip when the dispatcher has already derived the SID string
    /// for ACE targeting. The `PSID` itself is still derived inside
    /// [`create_app_container_sid`] at run time — see
    /// [`Self::preset_sid_string`].
