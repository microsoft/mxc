...
impl AppContainerScriptRunner {
    ...
    fn run_internal_impl(
        ...
        // `job` must remain in scope for the duration of the wait so the
        // Job Object stays open. Its Drop closes the handle, but the kernel
        // keeps UI restrictions attached to the process for its lifetime.
        let _job_guard = job;
