...
//! between Tier 1 (BaseContainer) or Tier 3 (AppContainer + DACL),
//! constructs the appropriate runner, and applies
...
//! [`DaclManager`] for every grant and deny path.
//!
//! The DACL cost is roughly O(N) Win32 syscalls plus one state-file
//! write per path in `readwrite_paths` ∪ `readonly_paths` ∪
//! `denied_paths`. The same number of syscalls is replayed in reverse on
//! `Drop`. At the typical N (6–12 paths) this adds tens of milliseconds
//! to both dispatch and shutdown; at larger N it scales linearly and can
//! add hundreds of milliseconds on each side. SDK callers that spawn
//! `wxc-exec` per task pay this cost on every invocation. Parent-directory
//! ACE rollup and session-scoped [`DaclManager`] caching are tracked as
//! follow-ups.
//!
...
mod tests {
    ...
    fn dispatch_warnings_propagated() {
        ...
        // and walks the probe chain, accumulating any "BaseContainer API
        // not present" warnings as appropriate on the test machine.
    ...
    fn dispatch_t1_runs_trivial_command_when_bc_present() {
        ...
        //
        // Other tests in this crate mutate `MXC_FORCE_TIER` under the
        // shared `ENV_LOCK`; we must acquire that lock with a
        // pass-through (unrecognized) value so the real probe chain
        // runs and we don't race those tests.
