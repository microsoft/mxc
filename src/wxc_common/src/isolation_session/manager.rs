...
impl IsolationSessionManager {
    ...
    pub(super) fn share_folders(
        ...
        logger: Option<&mut crate::logger::Logger>,
...
mod tests {
    ...
    // The `From<IsolationSessionConfigurationId> for IsoSessionConfigId`
    // impl is the sole bridge between MXC's internal enum and the WinRT
    // enum. If a new variant is added to either side without updating the
    // impl, these tests catch the drift.
