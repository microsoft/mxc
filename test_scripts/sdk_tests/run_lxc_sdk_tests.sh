#!/bin/bash
# Run LXC tests through the SDK CLI (end-to-end)
# This validates: SandboxPolicy → SDK → CLI → lxc-exec → LXC container
#
# Usage (from WSL):
#   sed -i 's/\r$//' test_scripts/sdk_tests/run_lxc_sdk_tests.sh  # fix line endings (once)
#   sudo bash test_scripts/sdk_tests/run_lxc_sdk_tests.sh
#
# Prerequisites:
