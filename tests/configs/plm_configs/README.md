...
| `fs_promoted.json`             | `cmd.exe` writes a file into `C:\plm_fs_test\readonly\` (pre-created by `run_plm_test.ps1`). The config pre-seeds that directory in `readonlyPaths`; PLM should observe the write and add the parent to `readwritePaths`, widening the policy from read-only to read+write. |
...
| `fs_add_readwrite.json`        | `cmd.exe` writes `C:\plm_fs_test\dst\out.txt` (dir pre-created). Config has no `filesystem` section; PLM should add the parent dir to `readwritePaths`. |
