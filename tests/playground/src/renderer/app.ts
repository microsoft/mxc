...
function buildRawBackendConfig(
  ...
  if (scenarioPolicy.network) {
    if (scenarioPolicy.network.allowedHosts || scenarioPolicy.network.blockedHosts) {
      throw new Error(
        'Schema 1.1 raw-backend configs require CIDR-based directional rules; '
        + 'hostname allow/block lists cannot be translated safely.',
      );
    }
    if (containment === 'hyperlight' && scenarioPolicy.network.enabled) {
      throw new Error(
        'Hyperlight has no representable schema 1.1 enabled-network posture.',
      );
    }
    // Hyperlight has no valid v1.1 network section. Omission keeps the
...
var ERROR_PATTERNS = [
  ...
  { pattern: 'CreateProcessSecurityEnvironment failed', title: 'Sandbox API failed', explanation: 'The process security environment API returned an error. Check the error code for details.', suggestion: 'Enable Debug Mode for more details.' },
