...
function Phase-UnitTests {
    ...
    Push-Location $CargoRoot
    try {
        $exit1 = Invoke-CargoTest -Arguments @('test', '-p', 'mxc-alpha-wxc-common', '--lib') -Label 'mxc-alpha-wxc-common --lib'
        Record-Result -Phase 'P7' -Name 'cargo test -p mxc-alpha-wxc-common' -Pass ($exit1 -eq 0) -Detail "exit=$exit1; full log: $CargoLog"
