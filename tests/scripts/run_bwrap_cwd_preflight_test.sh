...
run_expect_success "mount-boundary cwd execution failed." \
    "$LXC_EXEC" --experimental "$BASE/configs/bubblewrap_cwd_mount_boundary.json"
...
run_expect_success "root cwd execution failed." \
    "$LXC_EXEC" --experimental "$REPO_DIR/tests/configs/bubblewrap_cwd_root.json"
echo "$OUTPUT" | grep -q "CWD_ROOT_OK" \
    || fail "the sandbox root was not honored as the child's cwd."

echo "Running Bubblewrap virtual-filesystem descendant cwd test..."
run_expect_success "virtual-filesystem descendant cwd execution failed." \
    "$LXC_EXEC" --experimental \
    "$REPO_DIR/tests/configs/bubblewrap_cwd_virtual_descendant.json"
echo "$OUTPUT" | grep -q "could not be conclusively preflighted because it is beneath a synthetic virtual filesystem" \
    || fail "a virtual-filesystem descendant did not emit its advisory diagnostic."
echo "$OUTPUT" | grep -q "CWD_VIRTUAL_DESCENDANT_OK" \
    || fail "a runtime-created virtual-filesystem descendant was not honored as the child's cwd."

echo "Running Bubblewrap read-write cwd test..."
run_expect_success "read-write cwd execution failed." \
    "$LXC_EXEC" --experimental "$BASE/configs/bubblewrap_cwd_readwrite.json"
...
run_expect_success "synthetic-parent cwd execution failed." \
    "$LXC_EXEC" --experimental "$BASE/configs/bubblewrap_cwd_synthetic_parent.json"
...
run_expect_success "denied-mount synthetic-parent cwd execution failed." \
    "$LXC_EXEC" --experimental "$BASE/configs/bubblewrap_cwd_denied_parent.json"
...
run_expect_success "denied symlink-child cwd execution failed." \
    "$LXC_EXEC" --experimental \
    "$BASE/configs/bubblewrap_cwd_denied_symlink_parent.json"
...
run_expect_success "visible symlink-target cwd execution failed." \
    "$LXC_EXEC" --experimental "$BASE/configs/bubblewrap_cwd_symlink_visible.json"
