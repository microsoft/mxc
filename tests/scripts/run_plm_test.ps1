...
$AutomaticTests = @(
    ...
    @{
        ...
        # C:\plm_fs_test\src\. PLM should add the file path to
        # readonlyPaths (read events emit the file, not the parent).
        Name   = 'fs_add_readonly'
        Setup  = {
            New-Item -ItemType Directory -Path 'C:\plm_fs_test\src' -Force | Out-Null
            Set-Content -Path 'C:\plm_fs_test\src\input.txt' -Value 'ro source' -NoNewline
        }
        Cleanup = {
            Remove-Item -Recurse -Force 'C:\plm_fs_test' -ErrorAction SilentlyContinue
        }
        Expect = @(
            'filesystem.readonlyPaths[] += "C:\\plm_fs_test\\src'
        )
    },
    @{
    ...
    @{
        ...
        # C:\plm_fs_test\denied in deniedPaths; the workload
        ...
        Setup  = {
            New-Item -ItemType Directory -Path 'C:\plm_fs_test\denied' -Force | Out-Null
            Set-Content -Path 'C:\plm_fs_test\denied\secret.txt' -Value 'sensitive' -NoNewline
        }
        Cleanup = {
            Remove-Item -Recurse -Force 'C:\plm_fs_test' -ErrorAction SilentlyContinue
        }
...
foreach ($cfg in $configFiles) {
    ...
    try {
        ...
        if ($auditProc.WaitForExit($timeoutMs)) {
            ...
            # `src/host/plm/src/wpr_path.rs` was built to prevent.
