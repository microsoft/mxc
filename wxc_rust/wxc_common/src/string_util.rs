...
/// Convert a UTF-8 string to a COM BSTR.
pub fn to_bstr(input: &str) -> windows_core::BSTR {
    windows_core::BSTR::from(input)
}
...
mod tests {
    ...
    #[test]
    fn to_bstr_from_empty() {
        let bstr = to_bstr("");
        assert_eq!(bstr.len(), 0);
    }

    #[test]
    fn to_bstr_from_valid() {
        let bstr = to_bstr("Valid");
        assert_eq!(bstr.len(), 5);
        assert_eq!(bstr.to_string(), "Valid");
    }
}
